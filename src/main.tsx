...
} from "./hooks/useOnchainKitConfig";
...
const TermsAndConditionsPage = lazy(
  () => import("./pages/TermsAndConditionsPage.tsx"),
);
...
const CompetitionDetail = lazy(
  () => import("./components/CompetitionDetail.tsx"),
);

// Hero competition pages
const LamborghiniUrusPage = lazy(
  () => import("./pages/LamborghiniUrusPage.tsx"),
);
const BitcoinGiveawayPage = lazy(
...
const EntriesLayout = lazy(
  () => import("./components/UserDashboard/Entries/EntriesLayout.tsx"),
);
const EntriesList = lazy(
  () => import("./components/UserDashboard/Entries/EntriesList.tsx"),
);
const EntryDetail = lazy(
  () => import("./components/UserDashboard/Entries/EntryDetails.tsx"),
);
const CompetitionEntryDetails = lazy(
  () =>
    import("./components/UserDashboard/Entries/CompetitionEntryDetails.tsx"),
);
const Account = lazy(
  () => import("./components/UserDashboard/Account/AccountLayout.tsx"),
);
const NotificationsLayout = lazy(
  () =>
    import("./components/UserDashboard/Notifications/NotificationsLayout.tsx"),
);
...
const Promo = lazy(
  () => import("./components/UserDashboard/Promo/PromoLayout.tsx"),
);
const OrdersLayout = lazy(
  () => import("./components/UserDashboard/Orders/OrdersLayout.tsx"),
);
const OrdersList = lazy(
  () => import("./components/UserDashboard/Orders/OrdersList.tsx"),
);
const OrderDetails = lazy(
  () => import("./components/UserDashboard/Orders/OrderDetails.tsx"),
);

// Admin-only visual editors (secret routes)
const AuthModalVisualEditor = lazy(
  () => import("./pages/AuthModalVisualEditor.tsx"),
);
...
const router = createBrowserRouter([
  {
    ...
    children: [
      ...
      { path: "competitions", element: <CompetitionsPage /> },
      {
        ...
        element: <LamborghiniUrusPage />,
      },
      {
        path: "competitions/bitcoin-giveaway",
        element: <BitcoinGiveawayPage />,
      },
      ...
      },
      {
        ...
        children: [
          {
            index: true,
            element: <Navigate to="/dashboard/entries" replace />,
          },
          {
            path: "entries",
            ...
            children: [
              { index: true, element: <EntriesList /> },
              {
                ...
                element: <CompetitionEntryDetails />,
              },
...
if (!fallbackApiKey) {
  console.warn(
    "[main] VITE_CDP_API_KEY is not defined - will fetch from server",
  );
} else if (fallbackApiKey.length < 20) {
  console.warn(
    "[main] VITE_CDP_API_KEY appears to be invalid (too short) - will fetch from server",
  );
}

if (!cdpProjectId) {
  console.error(
    "[main] VITE_CDP_PROJECT_ID is not defined - Coinbase wallet creation will not work",
  );
} else {
  ...
  console.log(
    "[main] NOTE: Make sure this domain is whitelisted in CDP Portal at https://portal.cdp.coinbase.com/products/embedded-wallets/domains",
  );
}
 ...
 */
function OnchainKitProviderWrapper({
  children,
}: {
  ...
  useEffect(() => {
    if (serverConfig?.apiKey) {
      ...
      console.warn(
        "[OnchainKitProviderWrapper] Failed to load server config:",
        error,
      );
      if (fallbackApiKey) {
        console.log(
          "[OnchainKitProviderWrapper] Using fallback VITE_CDP_API_KEY",
        );
      } else {
        console.error(
          "[OnchainKitProviderWrapper] No API key available - OnchainKit features will not work",
        );
      }
    }
  ...
  if (!apiKey) {
    console.error(
      "[OnchainKitProviderWrapper] No OnchainKit API key available",
    );
    return <>{children}</>;
  }
