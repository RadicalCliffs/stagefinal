 ...
 */
export async function checkCompetitionSoldOut(
  ...
  try {
    ...
    const result = (await (supabase.rpc as any)(
      "check_and_mark_competition_sold_out" as any,
      {
    ...
    if (error) {
      ...
      console.log(
        "[checkCompetitionSoldOut] RPC not available or failed:",
        error.message,
      );
      return false;
    }
    ...
    if (data === true) {
      console.log(
        "[checkCompetitionSoldOut] Competition marked as sold out:",
        competitionId,
      );
      return true;
    }
...
export async function purchaseTicketsWithBalance({
  ...
  if (!userId || typeof userId !== "string" || userId.trim() === "") {
    console.error(
      "[purchaseTicketsWithBalance] Invalid userId provided:",
      userId,
    );
    return {
      success: false,
      error:
        "User identifier is missing. Please log in again and try once more.",
    };
  }
  ...
  try {
    ...
    if (!currentReservationId) {
      console.log(
        "[purchaseTicketsWithBalance] No reservation provided, creating one...",
      );
      ...
      if (!reserveResult.success || !reserveResult.data) {
        console.error(
          "[purchaseTicketsWithBalance] Reservation failed:",
          reserveResult.error,
        );
        return {
          success: false,
      ...
      console.log(
        "[purchaseTicketsWithBalance] Reservation created:",
    ...
    const ticketsToSend = reservationData
      ? reservationData.ticket_numbers
      : selectedTickets || [];

    // Validate we have ticket numbers to send
    if (!ticketsToSend || ticketsToSend.length === 0) {
      console.error(
        "[purchaseTicketsWithBalance] No ticket numbers available for purchase",
      );
      return {
        success: false,
    ...
    console.log(
      "[purchaseTicketsWithBalance] Purchasing with rolled-back contract:",
      {
    ...
    if (!purchaseResult.success || !purchaseResult.data) {
      console.error(
        "[purchaseTicketsWithBalance] Purchase failed:",
    ...
    console.log(
      "[purchaseTicketsWithBalance] Purchase successful:",
    ...
    try {
      const ticketNumbers =
        purchaseData.tickets?.map((t) => t.ticket_number) || [];
      supabase
        .from("competitions")
        .select("title")
        .eq("id", competitionId)
        ...
        .then(({ data: compData }: any) => {
          ...
          notificationService
            .notifyEntry(userId, competitionTitle, ticketNumbers, competitionId)
            .catch((err) => {
              console.warn(
                "[purchaseTicketsWithBalance] Failed to send entry notification:",
                err,
              );
            });
        });
      ...
      console.warn(
        "[purchaseTicketsWithBalance] Notification error (non-blocking):",
        notifyErr,
      );
    }
    ...
    checkCompetitionSoldOut(competitionId).catch((err: any) => {
      console.log(
        "[purchaseTicketsWithBalance] Sold-out check (non-blocking):",
        err,
      );
    });
 ...
 */
export async function getUserTicketsInCompetition(
  userId: string,
  competitionId: string,
) {
  try {
...
export async function getUserBalance(userId: string) {
  try {
    ...
    if (isTypeMismatchError) {
      console.warn(
        "[ticketPurchaseService] RPC type mismatch error - database migration may need to be applied. Falling back to direct query.",
      );
    } else {
      console.log(
        "[ticketPurchaseService] RPC returned 0 or failed, trying fallback queries:",
        rpcError?.message,
      );
    }
    ...
    const { data: subAccountData, error: subAccountError } = await supabase
      ...
      .eq("currency", "USD")
      .or(
        `canonical_user_id.eq.${canonicalUserId},user_id.eq.${normalizedUserId},privy_user_id.eq.${normalizedUserId}`,
      )
      .limit(1);
    ...
    console.log(
      "[ticketPurchaseService] No balance record found for user:",
...
export async function payWithBalance({
  ...
  if (
    !canonical_user_id ||
    typeof canonical_user_id !== "string" ||
    canonical_user_id.trim() === ""
  ) {
    console.error(
      "[payWithBalance] Invalid canonical_user_id provided:",
      canonical_user_id,
    );
    return {
      success: false,
  ...
  try {
      ...
      async () => {
        ...
        if (result.error) {
          ...
          if (
            ...
            console.warn(
              "[payWithBalance] Network error, will retry:",
              errorMessage,
            );
    ...
    if (
      ...
      data?.result?.new_balance !== undefined
    ) {
...
export async function executeBalancePaymentRPC({
  ...
  if (!userId || typeof userId !== "string" || userId.trim() === "") {
    console.error(
      "[executeBalancePaymentRPC] Invalid userId provided:",
      userId,
    );
    return {
      success: false,
  ...
  if (
    ...
    console.error(
      "[executeBalancePaymentRPC] Invalid competitionId provided:",
      competitionId,
    );
    return {
      success: false,
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    console.error(
      "[executeBalancePaymentRPC] Invalid amount provided:",
      amount,
    );
    return {
      success: false,
  ...
  if (!Number.isFinite(ticketCount) || ticketCount <= 0) {
    console.error(
      "[executeBalancePaymentRPC] Invalid ticketCount provided:",
      ticketCount,
    );
    return {
      success: false,
  ...
  try {
    console.log(
      "[executeBalancePaymentRPC] Calling execute_balance_payment RPC with:",
      {
    ...
    checkCompetitionSoldOut(competitionId).catch((err: Error) => {
      console.log(
        "[executeBalancePaymentRPC] Sold-out check (non-blocking):",
        err,
      );
    });
...
export async function finalizeBalancePayment({
  ...
  if (
    ...
    console.error(
      "[finalizeBalancePayment] Invalid reservationId provided:",
      reservationId,
    );
    return {
      success: false,
  ...
  try {
    console.log(
      "[finalizeBalancePayment] Calling finalize_purchase RPC with:",
      {
        reservationId,
      },
    );
    ...
    if (finalCompetitionId) {
      checkCompetitionSoldOut(finalCompetitionId).catch((err: Error) => {
        console.log(
          "[finalizeBalancePayment] Sold-out check (non-blocking):",
          err,
        );
      });
    }
