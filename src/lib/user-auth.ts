...
export const userAuth = {
  ...
  async getOrCreateUser(user: any): Promise<UserProfile | null> {
    ...
    if (walletAddress) {
      ...
      // Security Note: walletAddress is validated by isWalletAddress() check earlier (line 159)
      // and normalized by normalizeWalletAddress(). canonicalUserId is generated by toPrizePid()
      // which uses a controlled format (prize:pid:<validated_wallet>). Both are safe to use
      // in the .or() clause which requires string format for Supabase PostgREST API.
