...
class OmnipotentDataService {
  ...
  async initialize(userIdentifier?: string): Promise<void> {
    if (userIdentifier) {
      try {
        ...
        databaseLogger.error(
          "[OmnipotentData] Failed to resolve user identity",
          error,
        );
      }
    }
    ...
    if (hasAdminAccess()) {
      databaseLogger.info(
        "[OmnipotentData] Admin access available (schema validation disabled)",
      );
    } else {
   ...
   */
  private async ensureUserIdentity(
  ...
  async getCompetitions(
    ...
    try {
      ...
      databaseLogger.error(
        "[OmnipotentData] Failed to fetch competitions",
        error,
      );
      return [];
    }
  ...
  async getCompetition(
    ...
    options: DataFetchOptions = {},
  ): Promise<OmnipotentCompetition | null> {
    ...
    try {
      ...
      databaseLogger.error("[OmnipotentData] Failed to fetch competition", {
        competitionId,
        error,
      });
      return null;
  ...
  async getUserEntries(
    ...
    if (!identity) {
      databaseLogger.warn(
        "[OmnipotentData] Cannot fetch entries without user identity",
      );
      return [];
    }
    ...
    try {
      ...
      const validEntries = (Array.isArray(data) ? data : []).filter(
        (entry: any) => {
          ...
          if (
            ...
            databaseLogger.warn(
              "[OmnipotentData] Filtering out entry with missing competition_id",
              { entryId: entry.id },
            );
            return false;
          }
      ...
      const entries = validEntries.map((entry: any) =>
        this.transformEntry(entry, identity),
      );
      ...
      databaseLogger.error(
        "[OmnipotentData] Failed to fetch user entries",
        error,
      );
      return [];
    }
  ...
  async getCompetitionEntries(
    ...
    try {
      ...
      const { data: standardData, error: standardError } =
        await getCompetitionEntries(supabase, competitionId);
      ...
      if (!standardError && standardData) {
        ...
        console.log(
          "[OmnipotentData] Standard RPC unavailable, trying bypass_rls",
        );
      ...
      const entries = (data || []).map((entry) =>
        this.transformCompetitionEntry(entry),
      );
      ...
      databaseLogger.error(
        "[OmnipotentData] Failed to fetch competition entries",
        { competitionId, error },
      );
      return [];
    }
  ...
  private transformCompetitionEntry(raw: any): OmnipotentEntry {
    const ticketNumbers = this.parseTicketNumbers(
      raw.ticket_numbers || raw.ticketnumbers,
    );
    ...
    const isValidWallet = walletAddress && !/^0x0{40}$/i.test(walletAddress);
    ...
    return {
      ...
      username: isValidWallet
        ? this.formatUsername(walletAddress)
        : "Anonymous",
  ...
  async getUnavailableTickets(competitionId: string): Promise<number[]> {
    ...
    try {
      ...
      const { data: unavailableTickets, error: rpcError } =
        await getUnavailableTickets(supabase, competitionId);

      if (rpcError) {
        databaseLogger.warn(
          "[OmnipotentData] RPC get_unavailable_tickets failed, using fallback",
        ...
        if (pendingItemsData) {
          ...
          pendingItemsData.forEach((item: any) => {
            ...
            const isExpired =
              pending?.expires_at && new Date(pending.expires_at) < now;
            const validStatus =
              pending?.status &&
              ["pending", "confirming"].includes(pending.status);

            if (
              !isExpired &&
              validStatus &&
              typeof item.ticket_number === "number"
            ) {
        ...
        const pendingTicketsResult = (await supabase
          ...
          .in("status", ["pending", "confirming"])) as {
          data: any;
          error: any;
        };
        ...
        if (soldData) {
          soldData.forEach((row: any) => {
            ...
            if (Array.isArray(row.ticket_numbers)) {
              ...
              ticketNumbers = row.ticket_numbers.filter(
      ...
      const unavailable = Array.isArray(unavailableTickets)
        ? unavailableTickets
        : [];
      dataCache.set(cacheKey, unavailable, 5000); // Short cache for ticket availability
      return unavailable;
      ...
      databaseLogger.error(
        "[OmnipotentData] Failed to fetch unavailable tickets",
        { competitionId, error },
      );
      return [];
    }
  ...
  async getAvailableTickets(
    ...
    if (cached) {
      databaseLogger.debug(
        "[OmnipotentData] Returning cached available tickets",
        {
    ...
    try {
      ...
      const available = allTickets.filter(
        (ticket) => !unavailableSet.has(ticket),
      );
      ...
      databaseLogger.error(
        "[OmnipotentData] Failed to fetch available tickets",
        { competitionId, error },
      );
  ...
  private async cleanupExpiredReservations(
    ...
    databaseLogger.info(
      "[OmnipotentData] Cleanup is now handled by reserve_lucky_dip RPC",
      { competitionId },
    );
  }
  ...
  async reserveTicketsAggressive(
    ...
    databaseLogger.info(
      "[OmnipotentData] Aggressive ticket reservation started",
      {
        competitionId,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      if (attempt > 1) {
        ...
        databaseLogger.info("[OmnipotentData] Retrying reservation", {
          attempt,
          delay,
        });
      ...
      const result = await this.reserveTickets(
        userIdentifier,
        competitionId,
        currentSelection,
      );
      ...
      if (
        ...
        databaseLogger.warn("[OmnipotentData] Non-retryable error", {
          error: lastError,
        });
        return result;
      }
      ...
      if (lastError.includes("no longer available") && attempt < maxRetries) {
        databaseLogger.info(
          "[OmnipotentData] Tickets unavailable, fetching fresh pool",
          { attempt },
        );

        try {
          // Get fresh list of available tickets
          const freshAvailable = await this.getAvailableTickets(
            competitionId,
            totalTickets,
          );
          ...
          if (freshAvailable.length < ticketNumbers.length) {
            ...
            databaseLogger.warn(
              "[OmnipotentData] Insufficient tickets for reselection",
              {
          ...
          databaseLogger.info("[OmnipotentData] Reselected fresh tickets", {
            newSelection: (currentSelection.slice(0, 5) as any).concat(
          ...
          databaseLogger.error(
            "[OmnipotentData] Failed to fetch fresh tickets for reselection",
            err,
          );
  ...
  async reserveTickets(
    ...
    try {
      ...
      if (outOfRange.length > 0) {
        return {
          success: false,
          error: `Tickets out of range (max ${maxTicket}): ${outOfRange.join(", ")}`,
        };
      }
      ...
      if (pendingData) {
        pendingData.forEach((row: any) => {
          ...
          if (
            ...
            new Date((row as any).expires_at) < now
          ) {
            return;
          }
          ...
          if (
            normalizedRowUserId !== normalizedUserId &&
            Array.isArray(row.ticket_numbers)
          ) {
            row.ticket_numbers.forEach((n: number) => {
      ...
      const conflictingTickets = ticketNumbers.filter((t) =>
   ...
   */
  async refresh(
    type: "competitions" | "entries" | "all" = "all",
  ): Promise<void> {
