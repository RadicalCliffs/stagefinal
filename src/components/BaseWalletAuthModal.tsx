...
function validateNotTreasuryAddress(walletAddress: string): void {
  ...
  if (treasuryAddress && walletAddress.toLowerCase() === treasuryAddress) {
    throw new Error(
      "Invalid wallet address: Treasury address cannot be used as user wallet",
    );
  }
}
...
async function linkWalletToExistingUser(
  ...
  try {
    ...
    if (!email || !email.trim()) {
      console.error(
        "[BaseWallet] linkWalletToExistingUser called without email",
      );
    ...
    if (!walletAddress || !walletAddress.trim()) {
      console.error(
        "[BaseWallet] linkWalletToExistingUser called without wallet address",
      );
    ...
    const { data: walletUser, error: walletCheckError } = (await supabase
      ...
      .or(
        `base_wallet_address.ilike.${walletAddress},wallet_address.ilike.${walletAddress},eth_wallet_address.ilike.${walletAddress}`,
      )
    ...
    if (
      ...
      walletUser &&
      walletUser.email &&
      walletUser.email.toLowerCase() !== normalizedEmail
    ) {
      // Wallet is already linked to a different email account
      console.error(
        "[BaseWallet] Wallet already linked to different account:",
        walletUser.email,
      );
      throw new Error(
        `This wallet is already linked to ${walletUser.email}. Please use a different wallet or log in with that account.`,
      );
    }
    ...
    if (pendingLinkRequests.size >= MAX_PENDING_REQUESTS) {
      console.warn(
        "[BaseWallet] Pending requests Map exceeded limit, clearing old entries",
      );
    ...
    const requestPromise = (async () => {
      try {
        ...
        const pendingDataStr =
          localStorage.getItem("pendingSignupData") ||
        ...
        if (pendingDataStr) {
          try {
            ...
            console.log(
              "[BaseWallet] Found pending signup data with uid:",
              pendingSignupUid,
            );
          } catch (e) {
            console.error(
              "[BaseWallet] Failed to parse pending signup data:",
              e,
            );
          }
        }
        ...
        if (pendingSignupUid) {
          console.log(
            "[BaseWallet] Looking up user by uid from pending signup:",
            pendingSignupUid,
          );
          const result = (await supabase
            .from("canonical_users")
            .select(
              "id, uid, canonical_user_id, username, email, country, first_name, last_name",
            )
          ...
          if (existingUser) {
            console.log(
              ...
              existingUser.canonical_user_id,
            );
          }
        }
        ...
        if (!existingUser) {
          console.log(
            "[BaseWallet] Looking up user by email:",
            normalizedEmail,
          );
          const result = (await supabase
            .from("canonical_users")
        ...
        if (existingUser) {
          console.log(
            "[BaseWallet] Found existing user, updating with wallet. uid:",
            existingUser.uid,
          ...
          const { error: updateError } = await (
            supabase.from("canonical_users") as any
          )
            .update({
          ...
          if (updateError) {
            console.error(
              "[BaseWallet] Error updating user with wallet:",
              updateError,
            );
            return {
              success: false,
              error: `Database error: ${updateError.message || "Failed to update user"}`,
            };
          }
          ...
          try {
            ...
            console.log(
              "[BaseWallet] Calling attach_identity_after_auth RPC for existing user",
            );
            ...
            if (rpcError) {
              console.warn(
                "[BaseWallet] attach_identity_after_auth RPC warning:",
                rpcError,
              );
            } else {
              console.log(
                "[BaseWallet] attach_identity_after_auth RPC success",
              );
            }
          } catch (rpcErr) {
            console.warn(
              "[BaseWallet] attach_identity_after_auth RPC exception:",
              rpcErr,
            );
          }
          ...
          try {
            ...
            const { error: upsertError } = await (supabase.rpc as any)(
            ...
            if (upsertError) {
              console.warn(
                "[BaseWallet] upsert_canonical_user RPC warning:",
                upsertError,
              );
            } else {
            ...
            console.warn(
              "[BaseWallet] upsert_canonical_user RPC exception:",
              upsertErr,
            );
          }

          console.log(
            "[BaseWallet] Successfully linked wallet to existing user:",
            existingUser.id,
          );
        ...
        console.log(
          "[BaseWallet] User not found by email, checking by wallet address",
        );
        ...
        const { data: existingByWallet } = (await supabase
          ...
          .or(
            `wallet_address.ilike.${walletAddress.toLowerCase()},base_wallet_address.ilike.${walletAddress.toLowerCase()}`,
          )
          .maybeSingle()) as any;

        if (existingByWallet) {
          console.log(
            "[BaseWallet] Found user by wallet address:",
            existingByWallet.id,
          );
          ...
          if (updateError) {
            console.warn(
              "[BaseWallet] Error updating wallet user:",
              updateError,
            );
          }
          ...
          try {
            console.log(
              "[BaseWallet] Calling upsert_canonical_user RPC for wallet found by address",
            );
        ...
        if (profileData?.username || normalizedEmail) {
          console.log(
            "[BaseWallet] User not found, creating via upsert-user edge function",
          );
          ...
          try {
            const upsertResponse = await fetch(
              `${supabaseUrl}/functions/v1/upsert-user`,
              {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                  username:
                    profileData?.username || normalizedEmail.split("@")[0],
            ...
            if (!upsertResponse.ok) {
              console.error(
                "[BaseWallet] Failed to create user via upsert:",
                responseData,
              );
              return {
                success: false,
                error: responseData?.error || "Failed to create user account",
              };
            }

            console.log(
              "[BaseWallet] User created successfully via upsert:",
              responseData,
            );
            return {
              success: true,
        ...
        console.error(
          "[BaseWallet] Error in linkWalletToExistingUser inner promise:",
          innerError,
        );
        return {
          success: false,
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Unexpected error linking wallet",
    };
  }
}
 ...
 */
async function saveUserWithProfile(
  email: string,
  walletAddress: string,
  profile: ProfileData,
): Promise<boolean> {
  try {
    console.log("[BaseWallet] Saving user with profile:", {
      email,
      walletAddress,
      profile,
    });
    ...
    if (existingUser) {
      ...
      const { error }: any = await (supabase.from("canonical_users") as any)
        .update({
          wallet_address: walletAddress.toLowerCase(),
          base_wallet_address: walletAddress.toLowerCase(),
          eth_wallet_address: walletAddress.toLowerCase(),
    ...
    if (saveSuccess) {
      try {
        console.log(
          "[BaseWallet] Calling attach_identity_after_auth RPC after profile completion",
        );
      ...
      try {
        console.log(
          "[BaseWallet] Calling upsert_canonical_user RPC after profile completion",
        );
        ...
        const { error: upsertError } = await (supabase.rpc as any)(
          "upsert_canonical_user",
          {
            ...
            p_last_name:
              profile.fullName?.split(" ").slice(1).join(" ") || null,
...
export const BaseWalletAuthModal: React.FC<BaseWalletAuthModalProps> = ({
  ...
  const effectiveWalletAddress =
    hookWalletAddress ||
    storedWalletAddress ||
    localStorage.getItem("cdp:wallet_address");
  ...
  const walletPollIntervalRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (options?.isReturningUser || options?.connectExisting) {
        ...
        console.log(
          "[BaseWallet] Routing to wallet-choice (returning user flow)",
        );
  ...
  useEffect(() => {
    const handleCDPSignInSuccess = async () => {
      ...
      if (
        ...
        console.log(
          "[BaseWallet] CDP signed in but waiting for wallet address...",
        );
        ...
        walletPollIntervalRef.current = setInterval(() => {
          pollCount++;
          console.log(
            `[BaseWallet] Polling for wallet address... (${pollCount}/${maxPolls})`,
          );
      ...
      if (
        ...
        if (!pendingData && !pendingDataStr) {
          ...
          console.log(
            "[BaseWallet] No pending signup data found initially, attempting quick retries...",
          );
          while (!pendingDataStr && retryCount < PENDING_DATA_MAX_RETRIES) {
            await new Promise((resolve) =>
              setTimeout(resolve, PENDING_DATA_RETRY_DELAY_MS),
            );
          ...
          if (pendingDataStr) {
            try {
              pendingData = JSON.parse(pendingDataStr);
              console.log(
                "[BaseWallet] Found pending signup data after retry:",
                pendingData,
              );
            } catch (e) {
              console.error(
                "[BaseWallet] Failed to parse pending signup data after retry:",
                e,
              );
            }
          } else {
            console.log(
              "[BaseWallet] No pending signup data after retries - likely returning user or direct wallet connection",
            );
          }
        }
        ...
        const cdpEmail =
          ...
          (currentUser as any).linkedAccounts?.find(
            (a: any) => a.type === "email",
          )?.email;
        ...
        const effectiveEmail =
          pendingData?.profileData?.email || pendingData?.email || cdpEmail;
        ...
        if (effectiveEmail) {
          ...
          console.log("[BaseWallet] CDP sign-in successful:", {
            ...
            wallet: evmAddress,
            source: pendingData?.profileData?.email
              ? "pendingSignupData"
              : "CDP",
          });
          ...
          if (pendingData?.profileData) {
            console.log(
              "[BaseWallet] Creating user with profile data from NewAuthModal + wallet",
            );
            ...
            if (!formProfileData.username?.trim()) {
              console.error(
                "[BaseWallet] CRITICAL: No username found in signup data!",
              );
              setEmailError(
                "Username missing from signup data. Please start over.",
              );
              profileCheckedRef.current = false;
              return;
            }
            ...
            try {
              ...
              try {
                ...
                console.warn(
                  "[BaseWallet] Network error calling upsert-user, continuing with wallet:",
                  fetchError,
                );
                // Try to save directly to Supabase as fallback
                const directResult = await linkWalletToExistingUser(
                  userEmail,
                  evmAddress,
                  {
                    username: formProfileData.username,
              ...
              try {
                ...
                console.warn(
                  "[BaseWallet] Failed to parse upsert response, continuing:",
                  jsonError,
                );
              ...
              if (!upsertResponse.ok && upsertResponse.status !== 201) {
                console.error(
                  "[BaseWallet] Failed to create user:",
                  responseData,
                );
                throw new Error(
                  responseData?.error ||
            ...
            console.log(
              "[BaseWallet] No pending signup data found, attempting to link wallet to existing user",
            );
            ...
            if (result.success) {
              ...
              console.log(
                "[BaseWallet] Wallet linked successfully to existing user",
              );
              savedToDbRef.current = true;
              localStorage.setItem("cdp:wallet_address", evmAddress);
              ...
              console.log(
                "[BaseWallet] No existing user found, showing profile completion",
              );
          ...
          console.error(
            "[BaseWallet] CDP sign-in succeeded but no email found in currentUser or pendingSignupData",
          );
          console.log(
            "[BaseWallet] Showing profile completion form to collect missing data",
          );
    ...
    const handleWagmiConnection = async () => {
      if (
        ...
        if (timeSinceLastAttempt < DEBOUNCE_MS) {
          console.log(
            "[BaseWallet] Debouncing connection attempt, too soon after last attempt",
          );
          return;
        }
        ...
        const effectiveEmailForLinking =
          userEmail || pendingData?.profileData?.email || options?.email;
        ...
        console.log("[BaseWallet] Effective email for linking:", {
          ...
          isReturningUser:
            pendingData?.isReturningUser || options?.isReturningUser,
        });
        ...
        if (!effectiveEmailForLinking || !effectiveEmailForLinking.trim()) {
          ...
          setEmailError(
            "Unable to link wallet. Email is required. Please try logging in again.",
          );
        ...
        if (
          (pendingData?.isReturningUser || options?.isReturningUser) &&
          effectiveEmailForLinking
        ) {
          console.log(
            "[BaseWallet] Returning user flow - linking wallet to existing user",
          );
          ...
          if (result.success) {
            ...
            window.dispatchEvent(
              new CustomEvent("auth-complete", {
                detail: {
                  ...
                  email: effectiveEmailForLinking,
                },
              }),
            );
            ...
            setEmailError(
              result.error ||
                "Unable to connect wallet to account. Please try signing up again or contact support.",
            );
          }
          return;
        }
        ...
        if (pendingData?.profileData?.email) {
          console.log(
            "[BaseWallet] Creating user with profile data + external wallet",
          );
          ...
          try {
            ...
            try {
              ...
              console.warn(
                "[BaseWallet] Network error, trying direct link:",
                fetchError,
              );
              const directResult = await linkWalletToExistingUser(
                profileData.email,
          ...
          // CRITICAL FIX: Pass profile data so we can create user if they don't exist
          console.log(
            "[BaseWallet] Fallback - linking wallet to existing user by email",
          );
          ...
          if (result.success) {
            ...
            setEmailError(
              result.error ||
                "Unable to connect wallet. Please try again or contact support.",
            );
          }
        } else {
  ...
  useEffect(() => {
    if (flowState === "logged-in-success") {
      ...
      const walletForSuccess =
        effectiveWalletAddress || localStorage.getItem("cdp:wallet_address");

      console.log(
        "[BaseWallet] Success state reached, wallet:",
        walletForSuccess,
        ", scheduling auto-close in 2 seconds",
      );
  ...
  const handleCompleteProfile = useCallback(async () => {
    ...
    if (evmAddress && userEmail && !savedToDbRef.current) {
      savedToDbRef.current = true;
      const success = await saveUserWithProfile(
        userEmail,
        evmAddress,
        profileData,
      );
  ...
  const handleAuthenticate = useCallback(async () => {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, EVENT_PROCESSING_DELAY_MS),
    );
    onClose();
      ...
      <div className="bg-[#101010] border border-white/10 rounded-2xl p-6 w-full max-w-md relative max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="relative w-full py-3">
              ...
              <div className="relative flex justify-center text-xs">
                <span className="bg-[#101010] px-3 text-white/40">
            ...
            <p className="text-white/60 text-sm mb-2 text-center">
              Enter your email to verify and create your free Base wallet in one
              step.
            </p>
            ...
            {waitingForWallet ? (
              <div className="w-full py-8 flex flex-col items-center justify-center">
                <Loader2
                  className="animate-spin text-[#0052FF] mb-4"
                  size={40}
                />
                ...
                >
                  {(state: SignInState) => {
                    if (state.error) {
                      ...
                      if (
                        errorLower.includes("already linked") ||
                        errorLower.includes("already associated")
                      ) {
                        return (
                          <div
                            className="mt-2 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg"
                            role="alert"
                          >
                            <p className="text-yellow-400 text-xs text-center">
                              This email already has an account. Please enter
            ...
            <p className="mt-4 text-white/40 text-xs text-center">
              You won't have to do this again if you have your wallet saved on
              phone or desktop
            </p>
          </div>
        )}
        ...
        {flowState === "wallet-choice" && (
          <div className="flex flex-col">
            ...
            <p className="text-white/60 text-sm mb-6 text-center">
              Sign in, or create a Base Account to activate your secure wallet
              on ThePrize.
            </p>
                  ...
                  <div className="space-y-3">
                    ...
                    {isConnecting && (
                      <div className="p-4 bg-[#0052FF]/10 border border-[#0052FF]/30 rounded-lg text-center">
                        ...
                        <p className="text-white/60 text-xs">
                          Please complete the connection in your wallet app or
                          browser popup.
                        </p>
                      </div>
                    )}
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (cbConnector) {
                              ...
                              setEmailError(
                                "Coinbase Wallet connector not found. Please refresh and try again.",
                              );
                            }
                          }}
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (injectedConnector) {
                                  ...
                                  setEmailError(
                                    "No browser wallet detected. Please install a wallet extension.",
                                  );
                                }
                              }}
                              disabled={true}
                    ...
                    {isConnecting && connectionTimedOut && (
                      <div className="text-center py-4">
                        <Loader2
                          className="animate-spin text-[#0052FF] mx-auto"
                          size={32}
                        />
                      </div>
                    )}
            ...
            <div className="p-4 bg-white/5 rounded-lg text-center space-y-2">
              ...
              <p className="text-xs text-white/40">
                We never store your private keys. Your wallet is used for
                entries, top-ups, and ownership verification.
              </p>
            </div>
          </div>
        )}
        ...
        {flowState === "logged-in-success" && (
          <div className="flex flex-col items-center">
            ...
            <h2 className="text-white text-3xl font-bold mb-2 text-center">
              {textOverrides?.successTitle || "You're live."}
            </h2>
            ...
            {effectiveWalletAddress && (
              <div className="w-full bg-[#0052FF]/20 border border-[#0052FF] rounded-xl p-4 mb-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    onClick={handleCopy}
                    className="flex items-center gap-1 text-[#0052FF] text-xs"
                  >
                    {copied ? (
