...
const EntriesWithFilterTabs = ({
  competitionId,
  competitionUid,
}: EntriesWithFilterTabsProps = {}) => {
  ...
  const fetchCompetitionData = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "[EntriesWithFilterTabs] Error fetching competition data:",
        error,
      );
    }
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      ...
      const transformedEntries: Array<{
        ticketNumber: number;
        date: string;
        walletAddress: string;
        username?: string;
      ...
      try {
        ...
        const { data: standardData, error: standardError } =
          (await getCompetitionEntries(supabase, idToUse)) as any;
        ...
        if (
          ...
          rpcData.forEach((entry: any) => {
            ...
            if (
              ...
              entry.canonical_user_id &&
              entry.canonical_user_id.startsWith("prize:pid:")
            ) {
            ...
            const ticketNumbersStr =
              entry.ticketnumbers || entry.ticket_numbers;
            if (ticketNumbersStr) {
          ...
          entriesLogger.rpcError(
            "get_competition_entries",
            rpcError,
            "direct v_joincompetition_active query",
          );
          showDebugHintOnError();
      ...
      if (!rpcSucceeded || transformedEntries.length === 0) {
        ...
        if (jcData && jcData.length > 0) {
          entriesLogger.info(
            "Direct v_joincompetition_active query returned data",
            {
              count: jcData.length,
            },
          );
          ...
          jcData.forEach((entry: any) => {
            ...
            // canonical_user_id format is 'prize:pid:' + wallet_address
            let wallet =
              entry.wallet_address || entry.user_id || entry.userid || "";
            ...
            const ticketNumbersStr =
              entry.ticket_numbers || entry.ticketnumbers;
            if (ticketNumbersStr) {
              const ticketNumbers = ticketNumbersStr
                .split(",")
                .map((t: string) => parseInt(t.trim()))
                ...
                if (!seenTicketNumbers.has(ticketNum)) {
                  ...
                  transformedEntries.push({
                    ...
                    date: entry.purchasedate
                      ? new Date(entry.purchasedate).toLocaleString("en-US", {
                          year: "numeric",
                          month: "2-digit",
                          day: "2-digit",
                          hour: "2-digit",
      ...
      const { data: pendingData, error: pendingError } = await supabase
        ...
        .select(
          "ticket_numbers, created_at, canonical_user_id, wallet_address, transaction_hash, status, user_id",
        )
        .eq("competition_id", idToUse);
      ...
      if (!pendingError && pendingData && pendingData.length > 0) {
        ...
        pendingData.forEach((pending: any) => {
          ...
          if (
            ...
            pending.canonical_user_id.startsWith("prize:pid:")
          ) {
            ...
            try {
              ...
              if (Array.isArray(parsed)) {
                ticketNumbers = parsed
                  .map((t: string | number) => parseInt(String(t).trim()))
                  .filter((t: number) => !isNaN(t));
              }
            } catch {
            ...
            if (!seenTicketNumbers.has(ticketNum)) {
              ...
              transformedEntries.push({
                ...
                date: pending.created_at
                  ? new Date(pending.created_at).toLocaleString("en-US", {
                      year: "numeric",
                      month: "2-digit",
                      day: "2-digit",
                      hour: "2-digit",
      ...
      if (walletAddresses.size > 0) {
        try {
          ...
          const { data: usersData } = await supabase
            ...
            .select(
              "wallet_address, username, canonical_user_id, base_wallet_address",
            )
            .or(
              `wallet_address.in.(${walletArray.map((w) => `"${w}"`).join(",")}),base_wallet_address.in.(${walletArray.map((w) => `"${w}"`).join(",")})`,
            );
          ...
          transformedEntries.forEach((entry) => {
            if (
              entry.walletAddress &&
              entry.walletAddress.startsWith("prize:pid:")
            ) {
            ...
            if (canonicalUsersData && canonicalUsersData.length > 0) {
              canonicalUsersData.forEach((user: any) => {
                if (user.canonical_user_id) {
                  if (user.username) {
                    walletToUsername.set(
                      user.canonical_user_id.toLowerCase(),
                      user.username,
                    );
                  }
                  if (user.wallet_address) {
          ...
          if (joinData && joinData.length > 0) {
            ...
            if (userIds.size > 0) {
              ...
              const { data: moreUsersData } = await supabase
                ...
                .or(
                  `canonical_user_id.in.(${userIdArray.map((u) => `"${u}"`).join(",")}),wallet_address.in.(${userIdArray.map((u) => `"${u}"`).join(",")})`,
                );
              ...
              if (moreUsersData && moreUsersData.length > 0) {
                moreUsersData.forEach((user: any) => {
                  if (user.wallet_address) {
                    ...
                    if (
                      user.username &&
                      !walletToUsername.has(lowercaseWallet)
                    ) {
                      walletToUsername.set(lowercaseWallet, user.username);
                    }
                  }
                  if (user.canonical_user_id) {
                    ...
                    if (
                      user.wallet_address &&
                      !walletToAddress.has(lowercaseId)
                    ) {
          ...
          const entriesNeedingWallet = transformedEntries.filter(
            (e) =>
              !e.walletAddress ||
              e.walletAddress === "Unknown" ||
              !e.walletAddress.startsWith("0x"),
          );
          if (entriesNeedingWallet.length > 0) {
            const ticketNumbersToLookup = entriesNeedingWallet.map(
              (e) => e.ticketNumber,
            );

            const { data: ticketWalletData, error: ticketLookupError } =
              await supabase
                .from("tickets")
                .select(
                  "ticket_number, wallet_address, canonical_user_id, user_id",
                )
                .eq("competition_id", idToUse)
            ...
            if (
              ...
              ticketWalletData.forEach((t: any) => {
                ...
                if (
                  (!wallet || !wallet.startsWith("0x")) &&
                  t.canonical_user_id &&
                  t.canonical_user_id.startsWith("prize:pid:")
                ) {
              ...
              const newWallets = Array.from(walletAddresses).filter(
                (w) => !walletToUsername.has(w),
              );
              if (newWallets.length > 0) {
                const { data: newUsersData } = await supabase
                  ...
                  .or(
                    `wallet_address.in.(${newWallets.map((w) => `"${w}"`).join(",")}),base_wallet_address.in.(${newWallets.map((w) => `"${w}"`).join(",")})`,
                  );
                ...
                if (newUsersData && newUsersData.length > 0) {
                  newUsersData.forEach((user: any) => {
                    ...
                    if (user.base_wallet_address) {
                      const lowercaseBase =
                        user.base_wallet_address.toLowerCase();
                      if (user.username) {
                        walletToUsername.set(lowercaseBase, user.username);
                      }
                    }
              ...
              console.warn(
                "[EntriesWithFilterTabs] Ticket wallet lookup error:",
                ticketLookupError,
              );
            }
          }
          ...
          transformedEntries.forEach((entry) => {
            if (entry.walletAddress) {
              ...
              if (
                !entry.walletAddress.startsWith("0x") ||
      ...
      const entriesWithoutTxHash = transformedEntries.filter(
        (e) => !e.transactionHash,
      );
      if (entriesWithoutTxHash.length > 0) {
        try {
          ...
          const { data: competitionTxData } = await supabase
            .from("pending_tickets")
            .select(
              "canonical_user_id, wallet_address, user_id, transaction_hash, created_at",
            )
            .eq("competition_id", idToUse)
          ...
          entriesLogger.warn(
            "Failed to fetch competition tx hashes",
            txHashErr,
          );
        }
      }

      // ENHANCEMENT: For entries with balance_payment_* hashes, lookup actual top-up blockchain tx hash
      // This provides users with clickable on-chain verification of their balance top-up
      const entriesWithBalancePaymentHash = transformedEntries.filter(
        (e) =>
          e.transactionHash && e.transactionHash.startsWith("balance_payment_"),
      );

      if (entriesWithBalancePaymentHash.length > 0) {
        try {
          // Collect all wallet addresses that need top-up tx lookup
          const walletAddressesForTopup = new Set<string>();
          entriesWithBalancePaymentHash.forEach((entry) => {
            if (entry.walletAddress && entry.walletAddress.startsWith("0x")) {
              walletAddressesForTopup.add(entry.walletAddress.toLowerCase());
            }
          });

          if (walletAddressesForTopup.size > 0) {
            // Query user_transactions for topup transactions with real blockchain tx_id
            const walletArray = Array.from(walletAddressesForTopup);
            const { data: topupTxData } = (await supabase
              .from("user_transactions")
              .select("canonical_user_id, wallet_address, tx_id, created_at")
              .eq("type", "topup")
              .in("status", ["completed", "confirmed"])
              .not("tx_id", "is", null)
              .or(
                `wallet_address.in.(${walletArray.map((w) => `"${w}"`).join(",")}),canonical_user_id.in.(${walletArray.map((w) => `"prize:pid:${w}"`).join(",")})`,
              )
              .order("created_at", { ascending: false })) as any;

            if (topupTxData && topupTxData.length > 0) {
              // Build map of wallet address -> most recent topup tx_id
              const walletToTopupTx = new Map<string, string>();
              topupTxData.forEach((record: any) => {
                // Only use real blockchain hashes (0x prefixed, 66 chars)
                ...
                if (
                  ...
                  const canonicalWallet = record.canonical_user_id?.startsWith(
                    "prize:pid:",
                  )
                    ? record.canonical_user_id.substring(10).toLowerCase()
                    : null;

                  if (wallet && !walletToTopupTx.has(wallet)) {
                    walletToTopupTx.set(wallet, txId);
                  }
                  if (
                    canonicalWallet &&
                    !walletToTopupTx.has(canonicalWallet)
                  ) {
                    walletToTopupTx.set(canonicalWallet, txId);
                  }
                }
              });

              // Update entries with the real topup tx hash
              entriesWithBalancePaymentHash.forEach((entry) => {
                if (entry.walletAddress) {
                  const topupTx = walletToTopupTx.get(
                    entry.walletAddress.toLowerCase(),
                  );
                  if (topupTx) {
                    // Store original balance_payment ref in vrfHash, replace transactionHash with real tx
                    entry.vrfHash = entry.transactionHash; // Keep reference to original balance_payment_id
                    entry.transactionHash = topupTx;
                  }
                }
              });

              entriesLogger.info(
                "Enhanced balance payment entries with topup tx hashes",
                {
                  entriesEnhanced: entriesWithBalancePaymentHash.filter(
                    (e) =>
                      e.transactionHash && e.transactionHash.startsWith("0x"),
                  ).length,
                },
              );
            }
          ...
          entriesLogger.warn(
            "Failed to fetch topup tx hashes for balance payments",
            topupTxErr,
          );
        }
      }
        ...
        handlers: {
          onInsert: (payload) => {
            if (payload.new.competition_id === idToUse) {
              console.log(
                "[EntriesWithFilterTabs] New ticket detected, refreshing entries (debounced)",
              );
      ...
      },
      {
        ...
        handlers: {
          onInsert: (payload) => {
            if (payload.new.competition_id === idToUse) {
              console.log(
                "[EntriesWithFilterTabs] New transaction detected, refreshing entries (debounced)",
              );
              fetchEntriesDebounced();
            }
          },
  ...
  useEffect(() => {
    if (
      ...
      !filterOptions.find((opt) => opt.key === activeFilter?.key)
    ) {
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter(
      (e) => Number(e.ticketNumber) >= min && Number(e.ticketNumber) <= max,
    );
          ...
          {totalTabPages > 1 && (
            <div className="flex items-center justify-between bg-[#1a1a1a] border border-[#DDE404] rounded-lg p-4 mb-4">
              <div className="text-white sequel-45 text-sm">
                Showing ticket ranges{" "}
                {tabPage * MAX_TABS_PER_PAGE * TICKETS_PER_RANGE + 1} -{" "}
                {Math.min(
                  (tabPage + 1) * MAX_TABS_PER_PAGE * TICKETS_PER_RANGE,
                  maxTicketNumber,
                )}
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() =>
                    setTabPage((p) => Math.min(totalTabPages - 1, p + 1))
                  }
