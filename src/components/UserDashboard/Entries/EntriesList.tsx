...
export default function EntriesList() {
  ...
  const fetchEntries = useCallback(
    async (isBackgroundRefresh = false) => {
      if (!canonicalUserId) {
        console.warn(
          "[EntriesList] No canonical user ID available, skipping fetch",
        );
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const validEntries = (data || []).filter((entry: any) => {
          ...
          if (!tickets || tickets.trim() === "" || tickets === "0") {
            console.log(
              "[Dashboard.Entries] Filtering out invalid entry:",
              entry.id,
            );
            return false;
          }
          ...
          const competitionIds = [
            ...new Set(
              data
                ...
                .filter(
                  (id) =>
                    id && !id.startsWith("legacy-") && !id.startsWith("entry-"),
                ),
            ),
          ];
          if (competitionIds.length > 0) {
            console.log("[Dashboard.Entries] Syncing competition statuses:", {
              count: competitionIds.length,
            });
            // Run sync in background - don't await to avoid blocking UI
            database
              .syncStaleCompetitionStatuses(competitionIds)
              .then((result) => {
                if (result.updated.length > 0) {
                  // If any competitions were updated, refresh entries to get updated status
                  console.log(
                    "[Dashboard.Entries] Synced stale competitions, refreshing...",
              ...
              .catch((syncErr) => {
                ...
                console.warn(
                  "[Dashboard.Entries] Sync failed (non-critical):",
                  syncErr,
                );
              });
          }
  ...
  useEffect(() => {
    ...
    if (canonicalUserId) {
        ...
        .on(
          ...
          (payload) => {
            ...
            if (recordMatchesUser(record, normalizedWallet, canonicalUserId)) {
              console.log(
                "[EntriesList] Competition entry change detected:",
                payload.eventType,
              );
          ...
          (payload) => {
            ...
            if (recordMatchesUser(record, normalizedWallet, canonicalUserId)) {
              ...
              if (
                ...
                console.log(
                  "[EntriesList] User transaction completed:",
                  payload.eventType,
                  status,
                );
                debouncedFetchEntries();
        ...
        .on(
          ...
          (payload) => {
            ...
            if (
              ...
              record.metadata?.competition_id
            ) {
              console.log(
                "[EntriesList] Balance ledger purchase detected:",
                payload.eventType,
              );
              debouncedFetchEntries();
            }
      ...
      const winnersChannel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            if (recordMatchesUser(record, normalizedWallet, canonicalUserId)) {
              console.log(
                "[EntriesList] Winner update detected:",
                payload.eventType,
              );
              debouncedFetchEntries();
            }
      ...
      const handleBalanceUpdated = () => {
        console.log(
          "[EntriesList] Balance updated event detected, refreshing entries",
        );
        debouncedFetchEntries();
      };
  ...
  const filteredEntries = entries.filter((entry) => {
    ...
    const effectiveStatus =
      isCompetitionEnded && normalizedStatus === "live"
        ? "completed"
        : normalizedStatus;
  ...
  const groupedEntries = useMemo(() => {
    ...
    const deduplicatedEntries = filteredEntries.reduce((acc: any[], entry) => {
      ...
      const existingIndex = acc.findIndex((e: any) => {
        ...
        if (
          amountTimeDupeKey === existingAmountTimeKey &&
          entry.amount_spent > 0
        ) {
          return true;
        }
      ...
      if (existingIndex === -1) {
        ...
        const existingDataScore =
          (existing.ticket_numbers ? 1 : 0) +
          (existing.amount_spent > 0 ? 1 : 0) +
          (existing.transaction_hash ? 1 : 0);
        const newDataScore =
          (entry.ticket_numbers ? 1 : 0) +
          (entry.amount_spent > 0 ? 1 : 0) +
          (entry.transaction_hash ? 1 : 0);
    ...
    deduplicatedEntries.forEach((entry) => {
      ...
      const isCompetitionEnded =
        entry.end_date && new Date(entry.end_date) < new Date();
      const effectiveStatus =
        isCompetitionEnded && entry.status === "live"
          ? "completed"
          : entry.status;
      ...
      if (!groupMap.has(key)) {
        ...
        groupMap.set(key, {
          ...
          total_amount_spent: actualAmount,
          ...
          entry_ids: [entry.id],
          is_pending:
            entry.entry_type === "pending" || entry.status === "pending",
        ...
        const existingTickets = existing.all_ticket_numbers
          ? existing.all_ticket_numbers.split(",").map((t) => t.trim())
          : [];
        const newTickets = entry.ticket_numbers
          ? entry.ticket_numbers.split(",").map((t: string) => t.trim())
          : [];
        ...
        if (
          entry.transaction_hash &&
          entry.transaction_hash !== "no-hash" &&
          !existing.transaction_hashes.includes(entry.transaction_hash)
        ) {
        ...
        if (
          entry.expires_at &&
          (!existing.expires_at ||
            new Date(entry.expires_at) < new Date(existing.expires_at))
        ) {
    ...
    return (
      <div className="mt-10">
        <div className="text-center py-16 px-4">
          ...
          <p className="text-white/70 sequel-45 text-lg mb-4">{error}</p>
          <button
    ...
    <div className="mt-6 sm:mt-10">
      ...
      {isRefreshing && (
        <div className="mb-4 flex items-center justify-center gap-2 py-2 px-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
          ...
          <span className="text-blue-400 sequel-45 text-sm">
            Updating entries...
          </span>
        </div>
      )}
              ...
              return (
                ...
                >
                  <EntriesCard
                    ...
                    transactionHash={
                      entry.transaction_hashes.length > 0
                        ? entry.transaction_hashes[0]
                        : undefined
                    }
          ...
          <div className="col-span-2 text-center py-16 px-4">
            {activeTab.key === "live" ? (
              ...
              <>
                ...
                <p className="text-white/50 sequel-45 text-sm">
                  Competitions you've entered will appear here once they're
                  drawn
                </p>
              </>
            )}
          ...
          <div className="flex gap-1">
            {getPageNumbers().map((page, index) =>
              typeof page === "number" ? (
                ...
                <span
                  key={index}
                  className="w-6 sm:w-10 h-8 sm:h-10 flex items-center justify-center text-white/50 sequel-45 text-xs sm:text-sm"
                >
                  {page}
                </span>
              ),
      ...
      {groupedEntries.length > 0 && (
        <div className="text-center mt-3 sm:mt-4 text-white/60 sequel-45 text-xs sm:text-sm">
          Showing {startIndex + 1}-{Math.min(endIndex, groupedEntries.length)}{" "}
          of {groupedEntries.length} competitions
        </div>
      )}
    </div>
  );
