...
export const SendTransaction: React.FC<SendTransactionProps> = ({
  ...
  const addressError =
    recipientAddress && !isValidAddress ? "Invalid Ethereum address" : null;
  ...
  const canSend =
    isValidAddress && isValidAmount && !isSending && walletAddress;
        ...
        if (estimatedGas) {
          ...
          try {
            ...
            if (feeData.maxFeePerGas && feeData.maxPriorityFeePerGas) {
              ...
              console.warn(
                "Gas estimation returned null/undefined values, using fallback",
              );
              maxFeePerGas = FALLBACK_MAX_FEE_PER_GAS;
              maxPriorityFeePerGas = FALLBACK_MAX_PRIORITY_FEE_PER_GAS;
            }
            ...
            console.error(
              "Gas estimation failed, using fallback values:",
              gasEstimateError,
            );
    ...
    return (
      <div className="bg-[#1E1E1E] rounded-xl p-6 border border-red-500/30">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-white sequel-75 text-lg mb-2">
              Wallet Not Found
            </h3>
            <p className="text-white/60 sequel-45 text-sm">
              No embedded wallet address found. Please ensure you're signed in
              with a Base account.
            </p>
          </div>
        </div>
      ...
      <div className="bg-[#1E1E1E] rounded-xl p-6 border border-white/10">
        ...
        <div className="bg-[#2A2A2A] rounded-lg p-4 mb-4 space-y-3">
          <div>
            ...
            <p className="text-white sequel-45 text-sm font-mono">
          ...
          <div>
            <p className="text-white/40 sequel-45 text-xs mb-1.5">
              Transaction Hash
            </p>
            <p className="text-white sequel-45 text-xs font-mono break-all">
              {txHash}
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-green-400 sequel-75 text-sm">
        ...
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3 mt-4">
          <div className="flex items-start gap-2">
            ...
            <p className="text-blue-300/70 sequel-45 text-xs">
              Your transaction is being processed. It usually takes a few
              seconds to confirm on Base network.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="bg-[#1E1E1E] rounded-xl p-6 border border-white/10">
      <div className="flex items-start gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-white/60 sequel-45 text-sm">
            Transfer crypto from your{" "}
            {hasEmbeddedWallet ? "embedded" : "external"} wallet to another
            address.
          </p>
        </div>
      </div>
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mb-4 flex items-start gap-2">
          ...
          <div>
            <p className="text-red-400 sequel-75 text-sm mb-1">
              Transaction Failed
            </p>
      ...
      <div className="bg-[#2A2A2A] rounded-lg p-4 mb-4">
        <p className="text-white/40 sequel-45 text-xs mb-2">
          Your Wallet Address:
        </p>
        <p className="text-white sequel-45 text-sm font-mono break-all">
          {walletAddress}
        </p>
      </div>
      ...
      <div className="space-y-4 mb-6">
        <div>
          <div className="flex items-center justify-between mb-2">
            <label
              htmlFor="recipient"
              className="block text-white sequel-75 text-sm"
            >
              Recipient Address
            </label>
              ...
              {recentTransactions.map((tx, idx) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-white sequel-45 text-xs font-mono truncate">
                      {tx.to}
                    </p>
                    <p className="text-white/40 sequel-45 text-[10px] mt-0.5">
                      {tx.amount} ETH •{" "}
          ...
          />
          {addressError && (
            <p className="text-red-400 sequel-45 text-xs mt-1">
              {addressError}
            </p>
          )}
        </div>
        ...
        <div>
          <label
            htmlFor="amount"
            className="block text-white sequel-75 text-sm mb-2"
          >
            Amount (ETH)
          </label>
          <input
      ...
      {estimatedGas && isValidAmount && (
        <div className="bg-[#2A2A2A] rounded-lg p-4 mb-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-white/60 sequel-75 text-sm">
              Transaction Summary
            </p>
            {isEstimatingGas && (
          ...
          <div className="space-y-2 mb-3">
            <div className="flex items-center justify-between">
              <span className="text-white/60 sequel-45 text-sm">
                Send Amount
              </span>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-white sequel-75 text-sm">
                {estimatedGas.estimatedCost} ETH
              </span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-[#DDE404] sequel-75 text-lg">
                {(
                  parseFloat(amount) + parseFloat(estimatedGas.estimatedCost)
                ).toFixed(6)}{" "}
                ETH
              </span>
            </div>
          ...
          <p className="text-white/40 sequel-45 text-xs">
            Gas fees are paid to network validators. Base has some of the lowest
            fees of any network.
          </p>
        </div>
      )}
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <ul className="text-blue-300/70 sequel-45 text-xs space-y-1">
              ...
              <li>
                • Sending to the wrong address may result in permanent loss
              </li>
            </ul>
          </div>
        </div>
