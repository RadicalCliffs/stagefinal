...
const TicketSelector: React.FC<TicketSelectorProps> = ({
  ...
  const [selectedTickets, setSelectedTickets] = useState<number[]>(() => {
    try {
      ...
      try {
        ...
        console.warn(
          "Invalid ticket selection data in sessionStorage, clearing:",
          parseError,
        );
  ...
  const endRangeIndex = Math.min(
    startRangeIndex + RANGES_PER_PAGE,
  ...
  const fetchAvailableTickets = useCallback(
    async (showLoadingState = true) => {
      console.log("[TicketSelector] fetchAvailableTickets called", {
        showLoadingState,
        isInitialLoad,
      });
      ...
      try {
        ...
        const { data: unavailableData, error: unavailableError } =
          await supabase.rpc("get_unavailable_tickets", {
            competition_id: competitionId,
          } as any);

        if (unavailableError) {
          console.error(
            "[TicketSelector] Error fetching unavailable tickets:",
            unavailableError,
          );
        ...
        console.log("[TicketSelector] Setting availableTickets:", {
          count: available.length,
          first5: available.slice(0, 5),
        });
        ...
        console.warn(
          "[TicketSelector] RPC failed, using direct query fallback:",
          err,
        );
        ...
        try {
          ...
          console.log("[TicketSelector] Fallback succeeded:", {
            count: available.length,
          });
          setAvailableTickets(available);
          if (isInitialLoad) {
          ...
          if (isInitialLoad) {
            setLoadingError(
              "Failed to load available tickets. Please try again.",
            );
          }
        }
      } finally {
        console.log("[TicketSelector] fetchAvailableTickets finally", {
          showLoadingState,
          isInitialLoad,
        });
        if (showLoadingState && isInitialLoad) {
  ...
  const fetchOwnedTickets = useCallback(async () => {
    ...
    try {
      ...
      const ownedSet = await getOwnedTicketsForCompetition(competitionId, {
        ...
        privyId: canonicalUserId?.startsWith("did:privy:")
          ? canonicalUserId
          : undefined,
      });
    ...
    try {
      if (selectedTickets.length > 0) {
        sessionStorage.setItem(
          `selectedTickets_${competitionId}`,
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          console.log(
            "[TicketSelector] HTTP 409 - removing unavailable tickets:",
            parsedError.unavailableTickets,
          );
          ...
          const available = await database.getAvailableTicketsForCompetition(
            competitionId,
            totalTickets,
      ...
      if (response?.success !== true) {
        ...
        if (response?.unavailableTickets?.length > 0) {
          ...
          setAvailableTickets(available);
          throw new Error(
            `Tickets ${response.unavailableTickets.join(", ")} are no longer available. Please select different tickets.`,
          );
        }
      ...
      setReservationSuccess(
        "Tickets reserved! Complete payment within 30 seconds.",
      );
      return resId;
    } catch (err) {
  ...
  const handleCaptchaSuccess = async () => {
    ...
    if (resId) {
      ...
      console.warn(
        "[TicketSelectorWithTabs] Reservation returned null without setting an error",
      );
      setReservationError(
    ...
    <div className="max-w-5xl mx-auto border-[3px] border-[#DDE404] rounded-2xl text-white sm:p-8 p-4 space-y-8">
      ...
      <div className="space-y-4">
        ...
        {ownedTickets.length > 0 && (
          <div className="bg-[#1E3A2F] rounded-xl p-4 sm:p-6 border border-green-500/30">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-green-400 sequel-75 text-sm uppercase">
                Your Purchased Tickets ({ownedTickets.length})
              </h3>
            </div>
            ...
            <p className="text-green-300/60 sequel-45 text-xs mt-3">
              These tickets are already in your account. They will appear in
              your dashboard entries.
            </p>
          </div>
        )}
        ...
        <div className="bg-[#2A2A2A] rounded-xl p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex-1 w-full sm:w-auto">
              <h3 className="text-white sequel-75 text-sm mb-2">
                Your Selection
              </h3>
              <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto custom-scrollbar">
        ...
        {selectedTickets.length >=
          Math.min(availableTickets.length, MAX_TICKETS_PER_TRANSACTION) &&
          availableTickets.length > 0 && (
