...
const LoggedInUserBtn = ({ fullWidth = false }: LoggedInUserBtnProps) => {
  ...
  const primaryWallet =
    connectedWallets[primaryWalletIndex] || connectedWallets[0];
  ...
  const displayName = useMemo(() => {
    ...
    if (primaryEmail && primaryEmail !== "") {
      ...
      return primaryEmail.length > 20
        ? primaryEmail.slice(0, 17) + "..."
        : primaryEmail;
    }
  ...
  const {
    ...
    error: tokensError,
  } = useWalletTokens(primaryWallet?.address);
  ...
  const handleCopyAddress = async (address: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      console.warn(
        "[LoggedInUserBtn] Clipboard API failed, trying fallback:",
        err,
      );
      try {
  ...
  // Fetch unread count and set up real-time subscription
  useEffect(() => {
  ...
  return (
    <div className={`relative ${fullWidth ? "w-full" : ""}`} ref={dropdownRef}>
      ...
      >
        ...
        <div
          className={`flex items-stretch ${fullWidth ? "justify-between" : ""}`}
        >
          ...
          <div
            className={`flex flex-col justify-center px-2 sm:px-3 py-1 sm:py-1.5 ${fullWidth ? "flex-1" : ""}`}
          >
            <div className="flex items-center gap-1 sm:gap-1.5">
              <span
                className={`sequel-75 text-[#1A1A1A] text-[10px] sm:text-xs truncate ${fullWidth ? "max-w-none" : "max-w-20 sm:max-w-none"}`}
              >
                {displayName}
              </span>
              <ChevronRight
                size={12}
                className={`text-[#1A1A1A] transition-transform shrink-0 sm:w-3.5 sm:h-3.5 ${showDropdown ? "rotate-90" : ""}`}
              />
            </div>
            <div className="flex items-center gap-1 sm:gap-1.5 mt-0.5">
              <WalletCards
                size={12}
                className="text-[#1A1A1A] shrink-0 sm:w-3.5 sm:h-3.5"
              />
              <span className="sequel-95 text-[#1A1A1A] text-xs sm:text-sm">
        ...
        <>
          ...
          <div className="fixed sm:absolute inset-x-0 sm:inset-x-auto bottom-0 sm:bottom-auto sm:top-full sm:right-0 sm:mt-2 bg-[#1A1A1A] rounded-t-2xl sm:rounded-xl shadow-2xl z-100 w-full sm:w-85 md:w-95 lg:w-105 border border-[#2A2A2A] overflow-hidden max-h-[90vh] sm:max-h-[80vh] flex flex-col safe-area-inset-bottom">
            ...
            <div className="bg-linear-to-r from-[#2A2A2A] to-[#232323] p-3 sm:p-4 flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                <img
                  src={avatarUrl}
                  alt="avatar"
                  className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg object-cover border-2 border-[#DDE404] shrink-0"
                  crossOrigin="anonymous"
                  referrerPolicy="no-referrer"
                  onError={() => setAvatarError(true)}
                />
                <div className="flex-1 min-w-0">
                  <p className="sequel-95 text-white text-xs sm:text-sm truncate">
                    {displayName}
                  </p>
                  <p className="sequel-45 text-white/60 text-[10px] sm:text-xs mt-0.5 truncate">
                    {primaryEmail || truncateString(primaryWallet?.address, 12)}
                  </p>
                  <p className="sequel-45 text-[#DDE404] text-[10px] sm:text-xs mt-0.5">
                    {entryCount} active entries
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-3 sm:p-4 bg-[#DDE404]/5 border-b border-[#2A2A2A]">
              <p className="sequel-45 text-white/60 text-[10px] sm:text-xs uppercase mb-1 sm:mb-2">
                Play Balance
              </p>
            ...
            <div className="p-2 sm:p-3 grid grid-cols-2 gap-2 border-b border-[#2A2A2A]">
              ...
              >
                ...
                <span className="sequel-75 text-white text-[11px] sm:text-xs">
                  My Activity
                </span>
              </button>
              <button
            ...
            <div className="p-3 sm:p-4 flex-1 overflow-y-auto overscroll-contain">
              <div className="flex items-center justify-between mb-1">
                <span className="sequel-75 text-white text-xs sm:text-sm">
                  Connected Wallets
                </span>
                <span className="sequel-45 text-white/40 text-[10px] sm:text-xs">
                  {connectedWallets.length} connected
                </span>
              </div>
              <p className="sequel-45 text-white/50 text-[10px] sm:text-xs mb-2 sm:mb-3">
              ...
              {connectedWallets.length > 0 ? (
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            {!isPrimary && (
                              <button
                                onClick={(e) =>
                                  handleSetPrimaryWallet(index, e)
                                }
                          ...
                          <div className="mt-3 pt-3 border-t border-[#DDE404]/20">
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="sequel-75 text-white/70 text-[10px] uppercase">
                                  Wallet Tokens
                                </span>
                              </div>
                              <button
                              ...
                              >
                                <RefreshCw
                                  size={10}
                                  className={`text-white/40 hover:text-white ${tokensLoading ? "animate-spin" : ""}`}
                                />
                              </button>
                            </div>
                            ...
                            {tokensLoading ? (
                                    ...
                                    <div className="flex items-center gap-2">
                                      {token.logoUrl ? (
                                        ...
                                        <div className="w-5 h-5 rounded-full bg-[#404040] flex items-center justify-center">
                                          <span className="text-[8px] text-white/60 sequel-75">
                                            {token.symbol.slice(0, 2)}
                                          </span>
                                        </div>
                                      )}
                                      <span className="sequel-75 text-white text-[11px]">
                                        {token.symbol}
                                      </span>
                                    </div>
                                    <span className="sequel-45 text-white/70 text-[11px]">
                              ...
                              <div className="text-center py-2">
                                <p className="sequel-45 text-red-400/70 text-[10px]">
                                  Error loading tokens
                                </p>
                                <p className="sequel-45 text-white/30 text-[9px] mt-0.5">
                                  {tokensError}
                                </p>
                              </div>
                            ) : (
                              <p className="sequel-45 text-white/40 text-[10px] text-center py-2">
                                No tokens found
                              </p>
                            )}

                            {/* Security notice */}
                            <p className="sequel-45 text-white/40 text-[10px] sm:text-xs text-center mt-3">
                ...
                <div className="text-center py-6 bg-[#2A2A2A] rounded-lg">
                  ...
                  />
                  <p className="sequel-45 text-white/40 text-sm">
