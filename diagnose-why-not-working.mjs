...
async function main() {
  ...
  for (const [userId, nums] of Object.entries(byUser)) {
    const shortId = userId.includes(':') ? userId.split(':').pop().slice(0, 8) : userId;
    console.log(`      ${shortId}: ${nums.length} tickets (${nums[0]}-${nums[nums.length-1]})`);
  }
  ...
  const manualNumbers = manualTickets?.map(t => t.ticket_number).sort((a, b) => a - b) || [];
  ...
  console.log('\n' + '='.repeat(70));
  console.log('\n💡 DIAGNOSTIC RESULTS:');
  console.log('   If function returns data but tickets still show as available:');
