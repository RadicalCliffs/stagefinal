-- ============================================================================
-- HOTFIX: Remove reference to non-existent ce.is_pending column
-- ============================================================================
-- The RPC function is failing because it references ce.is_pending which doesn't exist
...
BEGIN
  ...
  SELECT
    ...
    -- Entry status - FIXED: Removed reference to non-existent is_pending column
    'confirmed' AS entry_status,
    
    -- Individual purchases - group by purchased_at timestamp (bulk purchases have same timestamp)
    COALESCE(
      (
...
BEGIN
  ...
  RAISE NOTICE '========================================================';
  RAISE NOTICE '✅ HOTFIX APPLIED: RPC now returns % entries', v_test_count;
  RAISE NOTICE 'Dashboard should now show correct amounts!';
  RAISE NOTICE '========================================================';
END $$;
