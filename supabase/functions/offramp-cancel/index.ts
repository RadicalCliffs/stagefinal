...
/**
 * Coinbase Offramp Cancel Handler
 *
...
const DEFAULT_REDIRECT_URL = 'https://stage.theprize.io/dashboard/entries?status=cancelled';

Deno.serve(async (req: Request) => {
  const cors = corsHeaders(req);
  const requestId = crypto.randomUUID().slice(0, 8);

  console.log(`[offramp-cancel][${requestId}] Incoming request: method=${req.method}`);
  ...
  try {
    ...
    console.log(`[offramp-cancel][${requestId}] partnerUserRef=${partnerUserRef}, payoutId=${payoutId}, reason=${reason}`);

    // If we have transaction tracking info, update the database
    if (partnerUserRef || payoutId) {
        ...
        if (payoutId) {
          ...
          if (txByPrivyId) {
            ...
            if (userData?.privy_user_id) {
              ...
              console.warn(`[offramp-cancel][${requestId}] Could not find user for partnerUserRef: ${partnerUserRef}`);
            }
          }
        }
      }
    ...
    if (req.method === "GET") {
      ...
      redirectWithStatus.searchParams.set('offramp_status', 'cancelled');
      redirectWithStatus.searchParams.set('reason', reason);

      if (payoutId) {
        redirectWithStatus.searchParams.set('payout_id', payoutId);
      }
    ...
    if (req.method === "POST") {
      ...
      console.log(`[offramp-cancel][${requestId}] POST body:`, JSON.stringify(body));

      return new Response(
        JSON.stringify({
          success: true,
          message: 'Offramp cancellation acknowledged',
          status: 'cancelled',
          reason,
        }),
    ...
    console.error(`[offramp-cancel][${requestId}] Error:`, error);
