...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (chainId !== 8453)
      return res(500, {
        ok: false,
        error: `RPC is not Base mainnet. chainId=${chainId}`,
      });
    ...
    if (privateKey.length !== 66) {
      return res(500, {
        ok: false,
        error: `Invalid private key length: ${privateKey.length}. Expected 66 chars (0x + 64 hex)`,
      });
    }
    ...
    if (!/^0x[0-9a-fA-F]{64}$/.test(privateKey)) {
      return res(500, {
        ok: false,
        error: "Private key is not valid hex format",
      });
    }
    ...
    // Pull comps that need VRF draw:
    // 1. Competitions that have ended (end_date passed) with status active/ended/drawing
    // 2. OR sold_out competitions (don't need to wait for end_date)
    const now = new Date().toISOString();

    // Query 1: Competitions past end_date
    const { data: endedComps, error: endedError } = await supabase
      .from("competitions")
      .select(
        "id, onchain_competition_id, status, end_date, vrf_requested_at, vrf_draw_tx",
      )
      .lt("end_date", now)
    ...
    // Query 2: Sold out competitions (ready for draw immediately, regardless of end_date)
    ...
    if (endedError) return res(500, { ok: false, error: endedError.message });
    if (soldOutError)
      return res(500, { ok: false, error: soldOutError.message });
    ...
    const comps = Array.from(compMap.values()).slice(0, limit);

    if (!comps?.length)
      return res(200, {
        ok: true,
        message: "No competitions needing VRF request",
      });
    ...
    for (const c of comps) {
      ...
      if (!compId) {
        results.push({
          id: c.id,
          ok: false,
          error: "Missing competitions.onchain_competition_id",
        });
        continue;
      }
      ...
      if (drawn) {
        ...
        results.push({
          id: c.id,
          ok: true,
          note: "Already drawn on-chain; marked completed",
        });
        continue;
      }
