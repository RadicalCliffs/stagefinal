...
/**
 * Coinbase Offramp Session Initialization
 *
 * This edge function handles the creation of session tokens for the Coinbase Offramp widget.
 * It follows the official Coinbase Offramp API documentation:
 * - Generates a JWT for CDP API authentication
 * - Calls the Session Token API with required parameters including clientIp
 * - Returns a session token and offramp URL for the client to use
 *
...
const OFFRAMP_TOKEN_API = "https://api.developer.coinbase.com/onramp/v1/token";
...
/**
 * Create a session token by calling the Coinbase Offramp Token API
 *
 ...
 */
async function createOfframpSessionToken(
  addresses: AddressConfig[],
  clientIp: string,
  assets?: string[]
): Promise<string> {
  const jwt = await generateCdpJwt(OFFRAMP_TOKEN_API);
  ...
  console.log(`[offramp-init] Calling Coinbase Token API with clientIp: ${clientIp.substring(0, 8)}...`);

  const response = await fetch(OFFRAMP_TOKEN_API, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    console.error(`[offramp-init] Token API error: ${response.status} ${errorText}`);
...
/**
 * Build the offramp URL with session token and parameters
 * Per documentation: https://pay.coinbase.com/v3/sell/input?sessionToken=<token>&<other params>
 */
function buildOfframpUrl(
  sessionToken: string,
  params: Partial<OfframpInitRequest>
): string {
  const url = new URL(OFFRAMP_BASE_URL);
  ...
  if (params.sourceNetwork) {
    url.searchParams.set("defaultNetwork", params.sourceNetwork);
  }

  if (params.sourceAsset) {
    url.searchParams.set("defaultAsset", params.sourceAsset);
  }

  if (params.amount !== undefined) {
    url.searchParams.set("presetCryptoAmount", String(params.amount));
  }
...
Deno.serve(async (req: Request) => {
  ...
  console.log(`[offramp-init][${requestId}] Incoming request: method=${req.method}`);
  ...
  try {
    ...
    if (!sourceAddress || typeof sourceAddress !== "string") {
      return new Response(
        JSON.stringify({ success: false, error: "sourceAddress is required" }),
        { status: 400, headers: { "Content-Type": "application/json", ...cors } }
      );
    }

    // Validate address format (basic check for Ethereum-style addresses)
    if (!/^0x[a-fA-F0-9]{40}$/.test(sourceAddress)) {
      return new Response(
    ...
    console.log(`[offramp-init][${requestId}] Client IP: ${clientIp.substring(0, 8)}..., Address: ${sourceAddress.substring(0, 10)}...`);

    // Build the addresses array for the Token API
    const addresses: AddressConfig[] = [
      {
        address: sourceAddress,
        blockchains: [sourceNetwork],
      },
    ];

    // Create the session token
    const sessionToken = await createOfframpSessionToken(addresses, clientIp, assets);
    console.log(`[offramp-init][${requestId}] Session token created successfully`);

    // Build the offramp URL with all parameters
    const offrampUrl = buildOfframpUrl(sessionToken, {
      sourceNetwork,
      sourceAsset,
      fiatCurrency,
      amount,
    ...
    console.log(`[offramp-init][${requestId}] Offramp URL generated successfully`);
    ...
    console.error(`[offramp-init][${requestId}] Error:`, error);
