...
--       joincompetition_unique_user_competition"
-- ============================================================================
-- Root cause: Production has a UNIQUE INDEX on (canonical_user_id, competitionid).
-- When a user buys ADDITIONAL tickets for a competition they already entered,
-- the plain INSERT fails.
--
-- Fix: Check for an existing entry first. If one exists, UPDATE it to append
-- the new ticket numbers and add to the totals. If not, INSERT as before.
-- ============================================================================
...
BEGIN
  ...
  IF v_existing_entry_uid IS NOT NULL THEN
    ...
    RAISE NOTICE 'Existing entry found (uid=%): appending % new tickets', v_existing_entry_uid, array_length(v_final_tickets, 1);

    -- Merge ticket numbers: existing + new
    IF v_existing_tickets IS NOT NULL AND LENGTH(TRIM(v_existing_tickets)) > 0 THEN
      v_merged_tickets := v_existing_tickets || ',' || v_ticket_numbers_str;
    ELSE
      v_merged_tickets := v_ticket_numbers_str;
    END IF;
    ...
    BEGIN
      ...
      SET
        ...
        numberoftickets = COALESCE(v_existing_count, 0) + array_length(v_final_tickets, 1),
  ...
  ELSE
    ...
    EXCEPTION WHEN unique_violation THEN
      ...
      -- Handle by updating instead.
      RAISE NOTICE 'INSERT hit unique_violation, falling back to UPDATE';
...
COMMENT ON FUNCTION purchase_tickets_with_balance IS
  'Atomic balance payment. Handles the joincompetition_unique_user_competition constraint by checking for existing entries and using UPDATE (append tickets) instead of INSERT when a user already has an entry for the competition.';
