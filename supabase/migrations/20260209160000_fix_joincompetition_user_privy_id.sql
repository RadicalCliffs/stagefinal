-- ============================================================================
-- FIX: record "new" has no field "user_privy_id" error during purchase
-- ============================================================================
-- Root cause: The trigger util.trg_set_cuid_from_context() fires BEFORE INSERT
-- on joincompetition and references NEW.user_privy_id via COALESCE. However,
-- the joincompetition table only has privy_user_id, NOT user_privy_id.
-- This causes every insert into joincompetition to fail with:
--   "record "new" has no field "user_privy_id""
--
-- Fix strategy (belt AND suspenders):
-- 1. Add the missing user_privy_id column to joincompetition
-- 2. Recreate util.trg_set_cuid_from_context() to be defensive (only
--    reference columns that exist, not COALESCE across both naming variants)
-- 3. Update purchase_tickets_with_balance RPC to include canonical_user_id
--    in the joincompetition INSERT so the trigger skips resolution entirely
-- ============================================================================

-- STEP 1: Add user_privy_id column to joincompetition if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = 'user_privy_id'
  ) THEN
    ALTER TABLE public.joincompetition ADD COLUMN user_privy_id TEXT;
    RAISE NOTICE 'Added user_privy_id column to joincompetition';
  ELSE
    RAISE NOTICE 'joincompetition already has user_privy_id column';
  END IF;
END $$;

-- STEP 2: Recreate util.trg_set_cuid_from_context() to be safe
-- Instead of COALESCE(NEW.privy_user_id, NEW.user_privy_id) which fails
-- when a table doesn't have one of those columns, just use privy_user_id
-- since that's the standard column name on joincompetition.
-- We wrap the entire function in a DO block to handle the case where
-- the util schema doesn't exist in this environment.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'util') THEN
    EXECUTE $fn$
      ...
      $body$;
    $fn$;
    RAISE NOTICE 'Recreated util.trg_set_cuid_from_context() with defensive column access';
  ELSE
    RAISE NOTICE 'util schema does not exist, skipping trg_set_cuid_from_context recreation';
  END IF;
END $$;

-- STEP 3: Recreate purchase_tickets_with_balance to include canonical_user_id
-- in the joincompetition INSERT so the trigger skips resolution entirely
...
BEGIN
  ...
  -- CRITICAL FIX: Include canonical_user_id so that the
  -- trg_joincompetition_set_cuid trigger (which calls
  -- util.trg_set_cuid_from_context()) skips resolution
  -- and does NOT try to access NEW.user_privy_id.
  -- =====================================================
  ...
  EXCEPTION WHEN OTHERS THEN
    -- If the joincompetition insert fails, try minimal insert
...
COMMENT ON FUNCTION purchase_tickets_with_balance IS
  'Atomic balance payment. Fixed to include canonical_user_id in joincompetition INSERT to prevent trg_set_cuid_from_context trigger from accessing non-existent user_privy_id column.';
