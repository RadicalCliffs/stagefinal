-- ============================================================================
-- FIX: Balance Ledger Source Filter and Transaction ID Surfacing
-- ============================================================================
-- ISSUE: Balance ledger entries are not showing in dashboard because:
--   1. Dashboard RPC queries for source='purchase' but payments use 'ticket_purchase'
--   2. Transaction ID needs to surface everywhere (using balance_ledger.transaction_id)
--
-- SOLUTION:
--   - Update RPC queries to match BOTH source variants
--   - Ensure transaction_id is properly returned in dashboard queries
--   - Surface transaction_id from balance_ledger.transaction_id column
--
-- Date: 2026-01-27
-- ============================================================================
...
-- ============================================================================
-- PART 1: Update get_user_competition_entries to include both source values
-- ============================================================================
...
BEGIN
  ...
  ELSE
    ...
    -- FIX: Match BOTH 'purchase' and 'ticket_purchase' source values
    -- Also surface the transaction_id from the column or metadata
    SELECT
      bl.id AS id,
      ...
      'USD' AS currency,
      -- Surface transaction_id: prefer column, fallback to metadata
      COALESCE(
    ...
    -- FIX: Match BOTH source values used for purchases
    AND bl.source IN ('purchase', 'ticket_purchase')
    AND bl.amount < 0  -- Debits only
...
- orders table
- balance_ledger (for balance-based purchases with source IN (purchase, ticket_purchase))
FIX: Now matches both purchase and ticket_purchase source values for balance_ledger entries.';

-- ============================================================================
-- PART 2: Update get_comprehensive_user_dashboard_entries to fix source filter
-- ============================================================================
...
BEGIN
  ...
  -- FIX: Match BOTH 'purchase' AND 'ticket_purchase' source values
  -- Also properly surface transaction_id from column or metadata
  SELECT
    bl.id::TEXT AS id,
    ...
    bl.created_at::TIMESTAMPTZ AS purchase_date,
    -- FIX: Surface transaction_id from column, then fallback to metadata
    COALESCE(
      bl.transaction_id::TEXT,
      bl.metadata->>'transaction_id',
      bl.metadata->>'transaction_hash',
...
FIX: Includes balance_ledger entries where source IN (purchase, ticket_purchase).
FIX: Properly surfaces transaction_id from balance_ledger.transaction_id column or metadata.';
...
BEGIN
  RAISE NOTICE '=====================================================';
  RAISE NOTICE 'FIX: Balance Ledger Source Filter and Transaction ID';
  RAISE NOTICE '=====================================================';
  RAISE NOTICE '* Updated get_user_competition_entries RPC:';
  RAISE NOTICE '  - FIX: Now matches source IN (purchase, ticket_purchase)';
  RAISE NOTICE '  - FIX: Surfaces transaction_id from column then metadata';
  RAISE NOTICE '=====================================================';
