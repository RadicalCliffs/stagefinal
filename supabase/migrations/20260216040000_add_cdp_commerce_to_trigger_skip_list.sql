-- Add cdp_commerce to trigger skip list
-- 
-- cdp_commerce is an alternate name for Coinbase Commerce used by CDP (Coinbase Developer Platform)
-- Like coinbase_commerce, it is handled by the commerce-webhook which:
-- 1. Credits balance via credit_balance_with_first_deposit_bonus RPC directly
-- 2. Creates user_transactions record with payment_provider='cdp_commerce'
...
BEGIN
  ...
  IF NEW.payment_provider IN (
    ...
    'cdp_commerce',           -- CDP Commerce (alternate name for Coinbase Commerce)
...
- base_account, coinbase_commerce, cdp_commerce, privy_base_wallet, etc.
