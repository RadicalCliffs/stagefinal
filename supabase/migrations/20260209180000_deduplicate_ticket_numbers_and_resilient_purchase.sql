-- ============================================================================
-- FIX: Prevent duplicate ticket numbers in joincompetition rows
-- AND: Make randomized (lucky dip) purchases resilient - never fail unless
--      the competition is truly sold out
-- ============================================================================
--
-- Problem 1: When appending tickets to an existing joincompetition entry,
-- the same ticket number could appear more than once in the comma-separated
-- ticketnumbers field. This migration adds deduplication logic to the
-- purchase_tickets_with_balance RPC so that merged ticket lists are always
-- unique.
--
-- Problem 2: Randomized (lucky dip) ticket purchases could fail with
-- a "ticket unavailable" error if the pre-reserved tickets got taken by
-- another transaction between reservation and purchase. The RPC now detects
-- conflicts in randomized mode and automatically selects replacement tickets
-- from the available pool, so it only fails if the competition is fully
-- sold out.
-- ============================================================================

-- Helper function to deduplicate a comma-separated list of ticket numbers
CREATE OR REPLACE FUNCTION deduplicate_ticket_numbers(p_ticket_numbers TEXT)
RETURNS TEXT
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
  ...
  v_unique INTEGER[];
BEGIN
  IF p_ticket_numbers IS NULL OR TRIM(p_ticket_numbers) = '' THEN
    RETURN p_ticket_numbers;
  END IF;

  -- Parse comma-separated string into array, handling whitespace
  SELECT array_agg(DISTINCT n ORDER BY n)
  INTO v_unique
  FROM (
    SELECT CAST(TRIM(unnest(string_to_array(p_ticket_numbers, ','))) AS INTEGER) AS n
  ) parsed
  WHERE n IS NOT NULL;
  ...
  RETURN array_to_string(v_unique, ',');
END;
$$;

COMMENT ON FUNCTION deduplicate_ticket_numbers IS
  'Removes duplicate ticket numbers from a comma-separated string, returning a sorted, deduplicated string.';

-- ============================================================================
-- Recreate purchase_tickets_with_balance with:
-- 1. Deduplication of ticket numbers on merge
-- 2. Resilient lucky dip: auto-replace conflicting tickets
-- ============================================================================
...
COMMENT ON FUNCTION purchase_tickets_with_balance IS
  'Atomic balance payment with deduplication. Handles joincompetition_unique_user_competition constraint by upserting. Deduplicates ticket numbers when merging to prevent same number appearing more than once per row. Lucky dip mode only fails when competition is fully sold out.';
