-- ============================================================================
-- Improve Lucky Dip Randomization
-- ============================================================================
-- Issue: Ticket allocation uses offset-based ordering which can be predictable
-- Solution: Use pure random() ordering for truly random ticket selection
-- This ensures that ticket allocation is unpredictable and fair
-- ============================================================================

BEGIN;

-- Drop existing version to recreate with better randomization
DROP FUNCTION IF EXISTS allocate_lucky_dip_tickets_batch(TEXT, UUID, INTEGER, NUMERIC, INTEGER, TEXT, INTEGER[]) CASCADE;

-- ============================================================================
-- Create allocate_lucky_dip_tickets_batch with improved randomization
-- ============================================================================
...
'Batch allocation of random tickets with improved randomization.
Uses pure random() ordering for truly unpredictable ticket selection.
This ensures fair "lucky dip" behavior where each available ticket has equal probability.
Allocates up to 500 tickets per call with proper locking to prevent double allocation.';
