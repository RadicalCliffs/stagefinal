-- ============================================================================
-- NUCLEAR OPTION: Completely rewrite allocate function with NO old references
-- ============================================================================
...
BEGIN
  ...
  SELECT COALESCE(array_agg(DISTINCT num::INTEGER), ARRAY[]::INTEGER[]) INTO v_sold_from_jc
  FROM jc_sold
  WHERE num_text ~ '^\s*\d+\s*$'  -- Only numeric strings
  ...
  SELECT COALESCE(array_agg(num), ARRAY[]::INTEGER[]) INTO v_sold_from_pending
  FROM pending_nums;
  ...
  SELECT array_agg(n ORDER BY (n + v_random_offset) % v_total_tickets + random()) INTO v_available_tickets
  FROM generate_series(1, v_total_tickets) AS n
  WHERE n != ALL(v_all_unavailable);
...
BEGIN
  RAISE NOTICE '========================================================';
  RAISE NOTICE 'NUCLEAR REWRITE: allocate_lucky_dip_tickets_batch';
  RAISE NOTICE 'Uses ONLY competition_id - NO competitionid anywhere';
  RAISE NOTICE '========================================================';
END $$;
