-- ============================================================================  
-- HOTFIX 3: Rename function temporarily to bypass cache
-- ============================================================================

-- Drop old version completely
DROP FUNCTION IF EXISTS allocate_lucky_dip_tickets_batch CASCADE;

-- Create with new name first
CREATE FUNCTION allocate_lucky_dip_tickets_batch_v2(
  p_user_id TEXT,
...
BEGIN
  ...
  -- **CRITICAL: Uses competition_id column only**
...
-- Now create alias with original name
...
SET search_path = public
AS $$
  SELECT allocate_lucky_dip_tickets_batch_v2(p_user_id, p_competition_id, p_count, p_ticket_price, p_hold_minutes, p_session_id, p_excluded_tickets);
$$;

GRANT EXECUTE ON FUNCTION allocate_lucky_dip_tickets_batch_v2(TEXT, UUID, INTEGER, NUMERIC, INTEGER, TEXT, INTEGER[]) TO authenticated, service_role;
...
DO $$
BEGIN
  RAISE NOTICE 'Created allocate_lucky_dip_tickets_batch_v2 and alias';
END $$;
