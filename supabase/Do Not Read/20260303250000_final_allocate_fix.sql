-- ============================================================================
-- FINAL FIX: allocate_lucky_dip_tickets_batch - NO OLD REFERENCES
-- ============================================================================

DROP FUNCTION IF EXISTS allocate_lucky_dip_tickets_batch(TEXT, UUID, INTEGER, NUMERIC, INTEGER, TEXT, INTEGER[]) CASCADE;
DROP FUNCTION IF EXISTS allocate_lucky_dip_tickets_batch_v2(TEXT, UUID, INTEGER, NUMERIC, INTEGER, TEXT, INTEGER[]) CASCADE;
...
BEGIN
  ...
  -- **CRITICAL LINE: Uses competition_id column (NOT competitionid)**
  ...
  FROM (
    ...
    WHERE joincompetition.competition_id = allocate_lucky_dip_tickets_batch.p_competition_id
...
GRANT EXECUTE ON FUNCTION allocate_lucky_dip_tickets_batch(TEXT, UUID, INTEGER, NUMERIC, INTEGER, TEXT, INTEGER[]) TO authenticated, service_role, anon;

DO $$
BEGIN
  RAISE NOTICE 'allocate_lucky_dip_tickets_batch COMPLETELY RECREATED with competition_id';
END $$;
