...
export default async function handler(request: Request, _context: Context) {
  ...
  try {
    ...
    const { username, email, firstName, lastName, country, telegram, avatar } = body;

    if (!username || !email) {
      return new Response(
        JSON.stringify({ success: false, error: "Username and email are required" }),
    ...
    console.log(`[create-user][${requestId}] Creating user: ${normalizedUsername} (${normalizedEmail})`);
    ...
    if (!verifiedSessions || verifiedSessions.length === 0) {
      console.log(`[create-user][${requestId}] Email not verified: ${normalizedEmail}`);
      return new Response(
        JSON.stringify({ success: false, error: "Email not verified. Please verify your email first." }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    if (session.expires_at && new Date(session.expires_at) < new Date()) {
      console.log(`[create-user][${requestId}] Email verification expired for: ${normalizedEmail}`);
      return new Response(
        JSON.stringify({ success: false, error: "Email verification expired. Please verify your email again." }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    if (existingUsers && existingUsers.length > 0) {
      ...
      if (existing.email === normalizedEmail) {
        console.log(`[create-user][${requestId}] Email already registered: ${normalizedEmail}`);
        return new Response(
          JSON.stringify({ success: false, error: "Email already registered", existingUsername: existing.username }),
          { status: 409, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      if (existing.username?.toLowerCase() === normalizedUsername) {
        console.log(`[create-user][${requestId}] Username already taken: ${normalizedUsername}`);
        return new Response(
    ...
    const createResponse = await fetch(`${supabaseUrl}/rest/v1/canonical_users`, {
      method: "POST",
      headers: {
    ...
    if (!createResponse.ok) {
      ...
      console.error(`[create-user][${requestId}] Failed to create user:`, errorText);
      return new Response(
