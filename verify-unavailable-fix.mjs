...
async function main() {
  ...
  try {
    const { data: unavailable, error } = await supabase.rpc('get_unavailable_tickets', {
    ...
    if (error) {
      ...
      console.log(`\n   📝 ACTION REQUIRED:`);
      console.log(`      1. Open: https://supabase.com/dashboard/project/mthwfldcjvpxjtmrqkqm/sql/new`);
      console.log(`      2. Copy all contents from: FIX_UNAVAILABLE_TICKETS_NOW.sql`);
    ...
    if (tickets?.length === unavailable?.length) {
      ...
      console.log(`   All purchased tickets will now show as unavailable to other users.`);
      console.log(`   Users can no longer select tickets that have been purchased.`);
    } else {
      const missing = (tickets?.length || 0) - (unavailable?.length || 0);
      console.log(`\n   ⚠️  MISMATCH: ${missing} tickets not showing as unavailable`);
      console.log(`   This means some purchased tickets can still be selected by others.`);
    ...
    if (tickets && unavailable && tickets.length !== unavailable.length) {
      ...
      const missing = Array.from(ticketNumbers).filter(n => !unavailableNumbers.has(n));
