...
[substage.theprize.io](http://substage.theprize.io) Schema for reference.   
Beneath this Public schema is the Auth schema. Beneath that is the Realtime schema.  There is also util, but it only has functions, no tables.   
This schema is for context only and is not meant to be run.  
\-- Table order and constraints may not be valid for execution.

CREATE TABLE public.Prize\_Instantprizes (  
  UID uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  competitionId uuid NOT NULL,  
  prize text NOT NULL,  
  prize\_value numeric,  
  winningTicket integer NOT NULL,  
  winningWalletAddress text,  
  winningUserId text,  
  privy\_user\_id text,  
  wonAt timestamp with time zone,  
  claimed\_at timestamp with time zone,  
  created\_at timestamp with time zone DEFAULT now(),  
  human\_id text,  
  CONSTRAINT Prize\_Instantprizes\_pkey PRIMARY KEY (UID),  
  CONSTRAINT Prize\_Instantprizes\_competitionId\_fkey FOREIGN KEY (competitionId) REFERENCES public.competitions(id)  
);  
CREATE TABLE public.\_entries\_progress (  
  competition\_id text NOT NULL,  
  canonical\_user\_id text NOT NULL,  
  last\_ticket\_number integer,  
  last\_processed\_at timestamp with time zone NOT NULL DEFAULT now(),  
  CONSTRAINT \_entries\_progress\_pkey PRIMARY KEY (competition\_id, canonical\_user\_id)  
);  
CREATE TABLE public.\_payment\_settings (  
  key text NOT NULL,  
  value\_timestamp timestamp with time zone,  
  CONSTRAINT \_payment\_settings\_pkey PRIMARY KEY (key)  
);  
CREATE TABLE public.admin\_sessions (  
  ...
  created\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT admin\_sessions\_pkey PRIMARY KEY (id),  
  CONSTRAINT admin\_sessions\_admin\_id\_fkey FOREIGN KEY (admin\_id) REFERENCES public.admin\_users(id)  
);  
CREATE TABLE public.admin\_users (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  ...
  CONSTRAINT admin\_users\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.admin\_users\_audit (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  admin\_id uuid,  
  action text,  
  metadata jsonb,  
  created\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT admin\_users\_audit\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.balance\_ledger (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  canonical\_user\_id text,  
  transaction\_type text,  
  amount numeric,  
  currency text DEFAULT 'USD'::text,  
  balance\_before numeric,  
  balance\_after numeric,  
  reference\_id text UNIQUE,  
  description text,  
  created\_at timestamp with time zone DEFAULT now(),  
  top\_up\_tx\_id text,  
  CONSTRAINT balance\_ledger\_pkey PRIMARY KEY (id),  
  CONSTRAINT fk\_balance\_ledger\_canonical\_user FOREIGN KEY (canonical\_user\_id) REFERENCES public.canonical\_users(canonical\_user\_id)  
);  
CREATE TABLE public.bonus\_award\_audit (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  wallet\_address text,  
  canonical\_user\_id text CHECK (canonical\_user\_id IS NULL OR canonical\_user\_id \~ '^prize:pid:0x\[a-f0-9\]{40}$'::text),  
  amount numeric NOT NULL,  
  currency text NOT NULL DEFAULT 'USD'::text,  
  awarded\_at timestamp with time zone NOT NULL DEFAULT now(),  
  reason text NOT NULL,  
  sub\_account\_balance\_before numeric,  
  sub\_account\_balance\_after numeric,  
  note text,  
  CONSTRAINT bonus\_award\_audit\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.canonical\_users (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  canonical\_user\_id text UNIQUE CHECK (canonical\_user\_id IS NULL OR canonical\_user\_id \~ '^prize:pid:0x\[a-fA-F0-9\]{40}$'::text OR canonical\_user\_id \~ '^prize:pid:temp\[0-9\]+$'::text),  
  uid text NOT NULL DEFAULT (gen\_random\_uuid())::text UNIQUE,  
  privy\_user\_id text,  
  email text UNIQUE,  
  wallet\_address text UNIQUE,  
  base\_wallet\_address text UNIQUE,  
  eth\_wallet\_address text UNIQUE,  
  username text,  
  avatar\_url text,  
  usdc\_balance numeric NOT NULL DEFAULT 0,  
  bonus\_balance numeric NOT NULL DEFAULT 0,  
  has\_used\_new\_user\_bonus boolean NOT NULL DEFAULT false,  
  created\_at timestamp with time zone NOT NULL DEFAULT now(),  
  updated\_at timestamp with time zone NOT NULL DEFAULT now(),  
  smart\_wallet\_address text,  
  country text,  
  first\_name text,  
  last\_name text,  
  telegram\_handle text,  
  is\_admin boolean NOT NULL DEFAULT false,  
  auth\_provider text,  
  wallet\_linked text,  
  linked\_wallets jsonb DEFAULT '\[\]'::jsonb,  
  primary\_wallet\_address text,  
  CONSTRAINT canonical\_users\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.cdp\_event\_queue (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  event\_name text NOT NULL,  
  payload jsonb NOT NULL,  
  status text NOT NULL DEFAULT 'pending'::text,  
  created\_at timestamp with time zone NOT NULL DEFAULT now(),  
  CONSTRAINT cdp\_event\_queue\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.competition\_entries (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  canonical\_user\_id text NOT NULL,  
  competition\_id uuid NOT NULL,  
  wallet\_address text,  
  tickets\_count integer NOT NULL DEFAULT 0,  
  ticket\_numbers\_csv text,  
  amount\_spent numeric,  
  payment\_methods text,  
  latest\_purchase\_at timestamp with time zone,  
  is\_winner boolean,  
  prize\_tiers text,  
  created\_at timestamp with time zone NOT NULL DEFAULT now(),  
  updated\_at timestamp with time zone NOT NULL DEFAULT now(),  
  username text,  
  CONSTRAINT competition\_entries\_pkey PRIMARY KEY (id),  
  CONSTRAINT fk\_competition\_entries\_competition FOREIGN KEY (competition\_id) REFERENCES public.competitions(id)  
);  
CREATE TABLE public.competitions (  
  ...
  description text,  
  image\_url text,  
  ticket\_price numeric DEFAULT 1,  
  total\_tickets integer DEFAULT 100,  
  sold\_tickets integer DEFAULT 0,  
  status text DEFAULT 'upcoming'::text,  
  start\_time timestamp with time zone DEFAULT now(),  
  end\_time timestamp with time zone,  
  winner\_count integer DEFAULT 1,  
  prize\_description text,  
  vrfulfillment\_address text,  
  vrf\_subscription\_id integer,  
  created\_at timestamp with time zone DEFAULT now(),  
  updated\_at timestamp with time zone DEFAULT now(),  
  deleted boolean DEFAULT false,  
  max\_tickets\_per\_user\_percentage integer,  
  ...
  is\_instant\_win boolean DEFAULT false,  
  num\_winners integer,  
  prize\_type text,  
  prize\_value numeric,  
  tickets\_sold integer,  
  uid uuid DEFAULT gen\_random\_uuid(),  
  winning\_ticket\_count integer,  
  vrf\_request\_id text,  
  vrf\_status text DEFAULT 'pending'::text,  
  vrf\_tx\_hash text,  
  onchain\_competition\_id text,  
  vrf\_random\_words ARRAY,  
  vrf\_proof text,  
  winner\_address text,  
  start\_date timestamp with time zone DEFAULT now(),  
  vrf\_draw\_requested\_at timestamp with time zone,  
  vrf\_draw\_completed\_at timestamp with time zone,  
  vrf\_randomness jsonb,  
  vrf\_error text,  
  vrf\_completed\_at timestamp with time zone,  
  draw\_date timestamp with time zone,  
  CONSTRAINT competitions\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.confirmation\_incident\_log (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  incident\_id text NOT NULL,  
  source text NOT NULL,  
  error\_type text,  
  error\_message text,  
  error\_stack text,  
  request\_context jsonb DEFAULT '{}'::jsonb,  
  env\_context jsonb DEFAULT '{}'::jsonb,  
  function\_context jsonb DEFAULT '{}'::jsonb,  
  severity text DEFAULT 'error'::text,  
  status\_code integer,  
  created\_at timestamp with time zone DEFAULT now(),  
  created\_by text,  
  user\_id text,  
  competition\_id text,  
  endpoint text,  
  occurred\_at timestamp with time zone NOT NULL DEFAULT now(),  
  CONSTRAINT confirmation\_incident\_log\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.custody\_transactions (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  user\_id uuid,  
  transaction\_type text NOT NULL,  
  provider text NOT NULL,  
  currency text NOT NULL,  
  amount numeric NOT NULL,  
  status text NOT NULL,  
  reference\_id text,  
  blockchain\_tx\_hash text,  
  nowpayments\_id text,  
  nowpayments\_payment\_id text,  
  nowpayments\_order\_id text,  
  metadata jsonb,  
  ...
  privy\_user\_id text,  
  wallet\_address text,  
  CONSTRAINT custody\_transactions\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.email\_auth\_sessions (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  email text NOT NULL,  
  verification\_code text NOT NULL,  
  expires\_at timestamp with time zone NOT NULL,  
  verified\_at timestamp with time zone,  
  attempts integer DEFAULT 0,  
  created\_at timestamp with time zone DEFAULT now(),  
  used\_at timestamp with time zone,  
  CONSTRAINT email\_auth\_sessions\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.enqueue\_cdp\_event (  
  id bigint NOT NULL DEFAULT nextval('cdp\_outbox\_id\_seq'::regclass),  
  ...
  created\_at timestamp with time zone NOT NULL DEFAULT now(),  
  processed\_at timestamp with time zone,  
  CONSTRAINT enqueue\_cdp\_event\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.faqs (  
  ...
  created\_at timestamp with time zone DEFAULT now(),  
  updated\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT faqs\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.hero\_competitions (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  competition\_id uuid,  
  display\_order integer,  
  is\_active boolean DEFAULT true,  
  created\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT hero\_competitions\_pkey PRIMARY KEY (id),  
  CONSTRAINT hero\_competitions\_competition\_id\_fkey FOREIGN KEY (competition\_id) REFERENCES public.competitions(id)  
);  
CREATE TABLE public.instant\_win\_grids (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  competition\_id uuid NOT NULL UNIQUE,  
  grid\_width integer DEFAULT 4 CHECK (grid\_width \> 0),  
  grid\_height integer DEFAULT 5 CHECK (grid\_height \> 0),  
  total\_tickets integer NOT NULL CHECK (total\_tickets \> 0),  
  is\_active boolean DEFAULT true,  
  created\_at timestamp with time zone DEFAULT now(),  
  updated\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT instant\_win\_grids\_pkey PRIMARY KEY (id),  
  CONSTRAINT instant\_win\_grids\_competition\_id\_fkey FOREIGN KEY (competition\_id) REFERENCES public.competitions(id)  
);  
CREATE TABLE public.internal\_transfers (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  transfer\_id text NOT NULL UNIQUE,  
  from\_user\_id uuid NOT NULL,  
  to\_user\_id uuid NOT NULL,  
  amount numeric NOT NULL,  
  currency text NOT NULL,  
  description text,  
  ...
  created\_at timestamp with time zone DEFAULT now(),  
  updated\_at timestamp with time zone DEFAULT now(),  
  from\_canonical\_user\_id text,  
  from\_privy\_user\_id text,  
  from\_wallet\_address text,  
  to\_canonical\_user\_id text,  
  to\_privy\_user\_id text,  
  to\_wallet\_address text,  
  CONSTRAINT internal\_transfers\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.joincompetition (  
  ...
  privy\_user\_id text,  
  updated\_at timestamp with time zone NOT NULL DEFAULT now(),  
  CONSTRAINT joincompetition\_pkey PRIMARY KEY (id),  
  CONSTRAINT joincompetition\_canonical\_user\_id\_fkey FOREIGN KEY (canonical\_user\_id) REFERENCES public.canonical\_users(canonical\_user\_id),  
  CONSTRAINT joincompetition\_competitionid\_fkey FOREIGN KEY (competitionid) REFERENCES public.competitions(id)  
);  
CREATE TABLE public.joined\_competitions (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  user\_uid uuid,  
  competition\_id uuid,  
  number\_of\_tickets integer NOT NULL,  
  wallet\_address text,  
  join\_date timestamp with time zone NOT NULL,  
  created\_at timestamp with time zone DEFAULT now(),  
  canonical\_user\_id text CHECK (canonical\_user\_id IS NULL OR canonical\_user\_id \~ '^prize:pid:0x\[a-f0-9\]{40}$'::text),  
  privy\_user\_id text,  
  CONSTRAINT joined\_competitions\_pkey PRIMARY KEY (id),  
  CONSTRAINT joined\_competitions\_competition\_id\_fkey FOREIGN KEY (competition\_id) REFERENCES public.competitions(id)  
);  
...
CREATE TABLE public.order\_tickets (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  order\_id uuid,  
  ticket\_number text NOT NULL,  
  created\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT order\_tickets\_pkey PRIMARY KEY (id),  
  CONSTRAINT order\_tickets\_order\_id\_fkey FOREIGN KEY (order\_id) REFERENCES public.orders(id)  
);  
CREATE TABLE public.orders (  
  ...
  payment\_session\_id text,  
  payment\_url text,  
  payment\_tx\_hash text,  
  completed\_at timestamp with time zone,  
  canonical\_user\_id text NOT NULL,  
  ledger\_ref text,  
  transaction\_ref text,  
  source text,  
  source\_id uuid,  
  bonus\_amount numeric,  
  cash\_amount numeric,  
  bonus\_currency text,  
  user\_wallet\_address text,  
  user\_privy\_id text,  
  notes text,  
  error\_message text,  
  posted\_to\_balance boolean DEFAULT false,  
  is\_backfill boolean DEFAULT false,  
  purchase\_at timestamp with time zone,  
  unique\_order\_key text UNIQUE,  
  competition\_title text,  
  competition\_description text,  
  competition\_id uuid,  
...
CREATE TABLE public.participants (  
  ...
  canonical\_user\_id text CHECK (canonical\_user\_id IS NULL OR canonical\_user\_id \~ '^prize:pid:0x\[a-f0-9\]{40}$'::text),  
  privy\_user\_id text,  
  CONSTRAINT participants\_pkey PRIMARY KEY (id),  
  CONSTRAINT participants\_competition\_id\_fkey FOREIGN KEY (competition\_id) REFERENCES public.competitions(id)  
);  
CREATE TABLE public.partners (  
  ...
  is\_active boolean DEFAULT true,  
  created\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT partners\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.payment\_idempotency (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  idempotency\_key text NOT NULL UNIQUE,  
  user\_id text NOT NULL,  
  competition\_id uuid,  
  amount numeric NOT NULL,  
  ticket\_count integer NOT NULL,  
  result jsonb NOT NULL,  
  created\_at timestamp with time zone DEFAULT now(),  
  expires\_at timestamp with time zone DEFAULT (now() \+ '24:00:00'::interval),  
  canonical\_user\_id text,  
  CONSTRAINT payment\_idempotency\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.payment\_webhook\_events (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  provider text NOT NULL,  
  event\_type text NOT NULL,  
  ...
  payer\_address text,  
  CONSTRAINT payment\_webhook\_events\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.payments (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  reservation\_id uuid NOT NULL,  
  owner\_canonical\_id text NOT NULL,  
  ...
  CONSTRAINT payments\_reservation\_id\_fkey FOREIGN KEY (reservation\_id) REFERENCES public.reservations(id)  
);  
CREATE TABLE public.payments\_jobs (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  created\_at timestamp with time zone NOT NULL DEFAULT now(),  
  run\_after timestamp with time zone NOT NULL DEFAULT now(),  
  status text NOT NULL DEFAULT 'pending'::text,  
  attempts integer NOT NULL DEFAULT 0,  
  max\_attempts integer NOT NULL DEFAULT 5,  
  job\_type text NOT NULL,  
  payload jsonb NOT NULL,  
  CONSTRAINT payments\_jobs\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.pending\_ticket\_items (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  pending\_ticket\_id uuid NOT NULL,  
  competition\_id uuid NOT NULL,  
  ticket\_number integer NOT NULL,  
  status text NOT NULL DEFAULT 'pending'::text,  
  expires\_at timestamp with time zone NOT NULL,  
  created\_at timestamp with time zone NOT NULL DEFAULT now(),  
  CONSTRAINT pending\_ticket\_items\_pkey PRIMARY KEY (id),  
  CONSTRAINT pending\_ticket\_items\_pending\_ticket\_id\_fkey FOREIGN KEY (pending\_ticket\_id) REFERENCES public.pending\_tickets(id),  
  CONSTRAINT pending\_ticket\_items\_competition\_id\_fkey FOREIGN KEY (competition\_id) REFERENCES public.competitions(id)  
);  
CREATE TABLE public.pending\_tickets (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  user\_id text,  
  canonical\_user\_id text,  
  wallet\_address text,  
  competition\_id uuid,  
  status text DEFAULT 'pending'::text CHECK (status \= ANY (ARRAY\['pending'::text, 'expired'::text\])),  
  hold\_minutes integer DEFAULT 15,  
  expires\_at timestamp with time zone,  
  reservation\_id uuid DEFAULT gen\_random\_uuid(),  
  created\_at timestamp with time zone DEFAULT now(),  
  ticket\_count integer,  
  ticket\_price numeric DEFAULT 1,  
  total\_amount numeric DEFAULT 0,  
  session\_id text,  
  confirmed\_at timestamp with time zone,  
  updated\_at timestamp with time zone DEFAULT now(),  
  transaction\_hash text,  
  payment\_provider text,  
  ticket\_numbers ARRAY,  
  payment\_id text,  
  idempotency\_key text,  
  privy\_user\_id text,  
  user\_privy\_id text,  
  note text,  
  CONSTRAINT pending\_tickets\_pkey PRIMARY KEY (id),  
  CONSTRAINT pending\_tickets\_competition\_id\_fkey FOREIGN KEY (competition\_id) REFERENCES public.competitions(id),  
  CONSTRAINT fk\_pending\_tickets\_canonical\_user FOREIGN KEY (canonical\_user\_id) REFERENCES public.canonical\_users(canonical\_user\_id)  
);  
CREATE TABLE public.platform\_statistics (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  stat\_date date NOT NULL,  
  total\_users integer,  
  active\_competitions integer,  
  completed\_competitions integer,  
  total\_competitions integer,  
  total\_tickets\_sold integer,  
  total\_revenue\_usd numeric,  
  created\_at timestamp with time zone DEFAULT now(),  
  updated\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT platform\_statistics\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.profiles (  
  ...
  country text,  
  telegram\_handle text,  
  telephone\_number text,  
  prior\_signup\_payload jsonb,  
  CONSTRAINT profiles\_pkey PRIMARY KEY (id),  
  CONSTRAINT profiles\_user\_id\_fkey FOREIGN KEY (user\_id) REFERENCES public.canonical\_users(id)  
);  
CREATE TABLE public.purchase\_idempotency (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  idempotency\_key text NOT NULL UNIQUE,  
  ...
  result jsonb,  
  created\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT purchase\_idempotency\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.purchase\_requests (  
  request\_id uuid NOT NULL,  
  competition\_id uuid NOT NULL,  
  user\_id text NOT NULL,  
  reservation\_id uuid,  
  selected\_tickets ARRAY NOT NULL DEFAULT '{}'::integer\[\],  
  ticket\_count integer NOT NULL,  
  created\_at timestamp with time zone NOT NULL DEFAULT now(),  
  processed\_at timestamp with time zone,  
  result\_ticket\_ids ARRAY,  
  total\_cost numeric,  
  currency text,  
  ...
  status text NOT NULL DEFAULT 'pending'::text,  
  error\_message text,  
  reservation\_ref text,  
  canonical\_user\_id text,  
  CONSTRAINT purchase\_requests\_pkey PRIMARY KEY (request\_id)  
);  
...
CREATE TABLE public.rng\_logs (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  competition\_id uuid,  
  competition\_type text CHECK (competition\_type \= ANY (ARRAY\['instant\_win'::text, 'draw'::text\])),  
  user\_id text,  
  numbers\_generated ARRAY,  
  context text NOT NULL,  
  outcome text CHECK (outcome \= ANY (ARRAY\['winner'::text, 'not\_winner'::text, 'predetermined\_winner'::text, 'ticket\_generation'::text, 'verification\_code'::text, 'success'::text, 'error'::text\])),  
  is\_winner boolean DEFAULT false,  
  ticket\_id text,  
  timestamp timestamp with time zone DEFAULT now(),  
  source text DEFAULT 'client\_side'::text CHECK (source \= ANY (ARRAY\['edge\_function'::text, 'client\_side'::text, 'database'::text\])),  
  function\_name text,  
  security\_level text DEFAULT 'MEDIUM'::text CHECK (security\_level \= ANY (ARRAY\['HIGH'::text, 'MEDIUM'::text, 'LOW'::text\])),  
  created\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT rng\_logs\_pkey PRIMARY KEY (id),  
  CONSTRAINT rng\_logs\_competition\_id\_fkey FOREIGN KEY (competition\_id) REFERENCES public.competitions(id)  
);  
CREATE TABLE public.site\_metadata (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  ...
  created\_at timestamp with time zone DEFAULT now(),  
  updated\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT site\_metadata\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.site\_stats (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  label text NOT NULL,  
  value text NOT NULL,  
  icon text,  
  ...
  created\_at timestamp with time zone DEFAULT now(),  
  updated\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT site\_stats\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.sub\_account\_balances (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  user\_id text,  
  currency text,  
  available\_balance numeric,  
  pending\_balance numeric,  
  last\_updated timestamp with time zone,  
  canonical\_user\_id text,  
  privy\_user\_id text,  
  wallet\_address text,  
  canonical\_user\_id\_norm text,  
  CONSTRAINT sub\_account\_balances\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.testimonials (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  author\_name text NOT NULL,  
  ...
  is\_active boolean DEFAULT true,  
  created\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT testimonials\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.tickets (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  competition\_id uuid,  
  ticket\_number integer,  
  status text DEFAULT 'available'::text,  
  purchased\_by uuid,  
  purchased\_at timestamp with time zone,  
  order\_id uuid,  
  created\_at timestamp with time zone DEFAULT now(),  
  user\_id text,  
  ...
  privy\_user\_id text,  
  prize\_tier text,  
  pending\_ticket\_id uuid,  
  payment\_amount numeric,  
  payment\_tx\_hash text,  
  purchase\_date timestamp with time zone,  
  canonical\_user\_id text,  
  wallet\_address text,  
  payment\_provider text,  
  tx\_id text,  
  transaction\_hash text DEFAULT COALESCE(payment\_tx\_hash, tx\_id),  
  user\_privy\_id text,  
  CONSTRAINT tickets\_pkey PRIMARY KEY (id),  
  CONSTRAINT tickets\_competition\_id\_fkey FOREIGN KEY (competition\_id) REFERENCES public.competitions(id)  
);  
CREATE TABLE public.tickets\_sold (  
  competition\_id uuid NOT NULL,  
  ticket\_number integer NOT NULL,  
  purchaser\_id text NOT NULL,  
  sold\_at timestamp with time zone NOT NULL DEFAULT now(),  
  CONSTRAINT tickets\_sold\_pkey PRIMARY KEY (competition\_id, ticket\_number)  
);  
CREATE TABLE public.user\_notifications (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  user\_id uuid NOT NULL,  
  title text NOT NULL,  
  ...
  created\_at timestamp with time zone DEFAULT now(),  
  updated\_at timestamp with time zone DEFAULT now(),  
  CONSTRAINT user\_notifications\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.user\_transactions (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  user\_id text,  
  canonical\_user\_id text,  
  wallet\_address text,  
  ...
  tx\_ref text DEFAULT (metadata \-\>\> 'tx\_ref'::text),  
  payment\_provider text,  
  payment\_status text,  
  ticket\_count integer,  
  webhook\_ref text UNIQUE,  
  charge\_id text UNIQUE,  
  charge\_code text,  
  checkout\_url text,  
  updated\_at timestamp with time zone DEFAULT now(),  
  primary\_provider text,  
  fallback\_provider text,  
  provider\_attempts integer DEFAULT 0,  
  provider\_error text,  
  posted\_to\_balance boolean DEFAULT false,  
  completed\_at timestamp with time zone,  
  expires\_at timestamp with time zone,  
  method text,  
  ...
  network text,  
  notes text,  
  CONSTRAINT user\_transactions\_pkey PRIMARY KEY (id),  
  CONSTRAINT user\_transactions\_competition\_id\_fkey FOREIGN KEY (competition\_id) REFERENCES public.competitions(id)  
);  
CREATE TABLE public.users (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  user\_id text NOT NULL UNIQUE,  
  wallet\_address text UNIQUE,  
  email text UNIQUE,  
  privy\_id text UNIQUE,  
  created\_at timestamp with time zone NOT NULL DEFAULT now(),  
  updated\_at timestamp with time zone NOT NULL DEFAULT now(),  
  canonical\_user\_id text,  
  CONSTRAINT users\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.wallet\_balances\_table\_backup (  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  user\_id text NOT NULL,  
  canonical\_user\_id text NOT NULL UNIQUE CHECK (canonical\_user\_id IS NULL OR canonical\_user\_id \~ '^prize:pid:0x\[a-f0-9\]{40}$'::text),  
  wallet\_address text NOT NULL,  
  balance numeric NOT NULL DEFAULT 0,  
  pending\_balance numeric NOT NULL DEFAULT 0,  
  last\_updated timestamp with time zone NOT NULL DEFAULT now(),  
  CONSTRAINT wallet\_balances\_table\_backup\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE public.webhook\_logs (  
  id text NOT NULL DEFAULT (gen\_random\_uuid())::text,  
  webhook\_type text NOT NULL,  
  event\_type text,  
  event\_data jsonb,  
  result\_data jsonb,  
  status text,  
...
CREATE TABLE public.winners (  
  ...
  user\_id text,  
  wallet\_address text,  
  prize\_position integer DEFAULT 1,  
  prize\_amount numeric,  
  vrfulfillment\_address text,  
  vrf\_proof text,  
  ...
  prize\_value numeric,  
  country text,  
  prize\_claimed boolean DEFAULT false,  
  tx\_hash text,  
  is\_instant\_win boolean DEFAULT false,  
  is\_promoted boolean DEFAULT false,  
  isShow boolean DEFAULT true,  
  vrf\_request\_id text,  
  won\_at timestamp with time zone DEFAULT now(),  
  crdate text,  
  CONSTRAINT winners\_pkey PRIMARY KEY (id),  
  CONSTRAINT winners\_competition\_id\_fkey FOREIGN KEY (competition\_id) REFERENCES public.competitions(id)  
);
...
CREATE TABLE auth.audit\_log\_entries (  
  instance\_id uuid,  
  id uuid NOT NULL,  
  payload json,  
  created\_at timestamp with time zone,  
  ip\_address character varying NOT NULL DEFAULT ''::character varying,  
  CONSTRAINT audit\_log\_entries\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE auth.flow\_state (  
  id uuid NOT NULL,  
  user\_id uuid,  
  auth\_code text NOT NULL,  
  code\_challenge\_method USER-DEFINED NOT NULL,  
  code\_challenge text NOT NULL,  
  provider\_type text NOT NULL,  
  provider\_access\_token text,  
  provider\_refresh\_token text,  
  created\_at timestamp with time zone,  
  updated\_at timestamp with time zone,  
  authentication\_method text NOT NULL,  
  auth\_code\_issued\_at timestamp with time zone,  
  CONSTRAINT flow\_state\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE auth.identities (  
  provider\_id text NOT NULL,  
  user\_id uuid NOT NULL,  
  identity\_data jsonb NOT NULL,  
  provider text NOT NULL,  
  last\_sign\_in\_at timestamp with time zone,  
  created\_at timestamp with time zone,  
  updated\_at timestamp with time zone,  
  email text DEFAULT lower((identity\_data \-\>\> 'email'::text)),  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  CONSTRAINT identities\_pkey PRIMARY KEY (id),  
  CONSTRAINT identities\_user\_id\_fkey FOREIGN KEY (user\_id) REFERENCES auth.users(id)  
);  
CREATE TABLE auth.instances (  
  id uuid NOT NULL,  
  uuid uuid,  
  raw\_base\_config text,  
  created\_at timestamp with time zone,  
  updated\_at timestamp with time zone,  
  CONSTRAINT instances\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE auth.mfa\_amr\_claims (  
  session\_id uuid NOT NULL,  
  created\_at timestamp with time zone NOT NULL,  
  updated\_at timestamp with time zone NOT NULL,  
  authentication\_method text NOT NULL,  
  id uuid NOT NULL,  
  CONSTRAINT mfa\_amr\_claims\_pkey PRIMARY KEY (id),  
  CONSTRAINT mfa\_amr\_claims\_session\_id\_fkey FOREIGN KEY (session\_id) REFERENCES auth.sessions(id)  
);  
CREATE TABLE auth.mfa\_challenges (  
  id uuid NOT NULL,  
  factor\_id uuid NOT NULL,  
  created\_at timestamp with time zone NOT NULL,  
  verified\_at timestamp with time zone,  
  ip\_address inet NOT NULL,  
  otp\_code text,  
  web\_authn\_session\_data jsonb,  
  CONSTRAINT mfa\_challenges\_pkey PRIMARY KEY (id),  
  CONSTRAINT mfa\_challenges\_auth\_factor\_id\_fkey FOREIGN KEY (factor\_id) REFERENCES auth.mfa\_factors(id)  
);  
CREATE TABLE auth.mfa\_factors (  
  id uuid NOT NULL,  
  user\_id uuid NOT NULL,  
  friendly\_name text,  
  factor\_type USER-DEFINED NOT NULL,  
  status USER-DEFINED NOT NULL,  
  created\_at timestamp with time zone NOT NULL,  
  updated\_at timestamp with time zone NOT NULL,  
  secret text,  
  phone text,  
  last\_challenged\_at timestamp with time zone UNIQUE,  
  web\_authn\_credential jsonb,  
  web\_authn\_aaguid uuid,  
  last\_webauthn\_challenge\_data jsonb,  
  CONSTRAINT mfa\_factors\_pkey PRIMARY KEY (id),  
  CONSTRAINT mfa\_factors\_user\_id\_fkey FOREIGN KEY (user\_id) REFERENCES auth.users(id)  
);  
CREATE TABLE auth.oauth\_authorizations (  
  id uuid NOT NULL,  
  authorization\_id text NOT NULL UNIQUE,  
  client\_id uuid NOT NULL,  
  user\_id uuid,  
  redirect\_uri text NOT NULL CHECK (char\_length(redirect\_uri) \<= 2048),  
  scope text NOT NULL CHECK (char\_length(scope) \<= 4096),  
  state text CHECK (char\_length(state) \<= 4096),  
  resource text CHECK (char\_length(resource) \<= 2048),  
  code\_challenge text CHECK (char\_length(code\_challenge) \<= 128),  
  code\_challenge\_method USER-DEFINED,  
  response\_type USER-DEFINED NOT NULL DEFAULT 'code'::auth.oauth\_response\_type,  
  status USER-DEFINED NOT NULL DEFAULT 'pending'::auth.oauth\_authorization\_status,  
  authorization\_code text UNIQUE CHECK (char\_length(authorization\_code) \<= 255),  
  created\_at timestamp with time zone NOT NULL DEFAULT now(),  
  expires\_at timestamp with time zone NOT NULL DEFAULT (now() \+ '00:03:00'::interval),  
  approved\_at timestamp with time zone,  
  nonce text CHECK (char\_length(nonce) \<= 255),  
  CONSTRAINT oauth\_authorizations\_pkey PRIMARY KEY (id),  
  CONSTRAINT oauth\_authorizations\_client\_id\_fkey FOREIGN KEY (client\_id) REFERENCES auth.oauth\_clients(id),  
  CONSTRAINT oauth\_authorizations\_user\_id\_fkey FOREIGN KEY (user\_id) REFERENCES auth.users(id)  
);  
CREATE TABLE auth.oauth\_client\_states (  
  id uuid NOT NULL,  
  provider\_type text NOT NULL,  
  code\_verifier text,  
  created\_at timestamp with time zone NOT NULL,  
  CONSTRAINT oauth\_client\_states\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE auth.oauth\_clients (  
  id uuid NOT NULL,  
  client\_secret\_hash text,  
  registration\_type USER-DEFINED NOT NULL,  
  redirect\_uris text NOT NULL,  
  grant\_types text NOT NULL,  
  client\_name text CHECK (char\_length(client\_name) \<= 1024),  
  client\_uri text CHECK (char\_length(client\_uri) \<= 2048),  
  logo\_uri text CHECK (char\_length(logo\_uri) \<= 2048),  
  created\_at timestamp with time zone NOT NULL DEFAULT now(),  
  updated\_at timestamp with time zone NOT NULL DEFAULT now(),  
  deleted\_at timestamp with time zone,  
  client\_type USER-DEFINED NOT NULL DEFAULT 'confidential'::auth.oauth\_client\_type,  
  CONSTRAINT oauth\_clients\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE auth.oauth\_consents (  
  id uuid NOT NULL,  
  user\_id uuid NOT NULL,  
  client\_id uuid NOT NULL,  
  scopes text NOT NULL CHECK (char\_length(scopes) \<= 2048),  
  granted\_at timestamp with time zone NOT NULL DEFAULT now(),  
  revoked\_at timestamp with time zone,  
  CONSTRAINT oauth\_consents\_pkey PRIMARY KEY (id),  
  CONSTRAINT oauth\_consents\_user\_id\_fkey FOREIGN KEY (user\_id) REFERENCES auth.users(id),  
  CONSTRAINT oauth\_consents\_client\_id\_fkey FOREIGN KEY (client\_id) REFERENCES auth.oauth\_clients(id)  
);  
CREATE TABLE auth.one\_time\_tokens (  
  id uuid NOT NULL,  
  user\_id uuid NOT NULL,  
  token\_type USER-DEFINED NOT NULL,  
  token\_hash text NOT NULL CHECK (char\_length(token\_hash) \> 0),  
  relates\_to text NOT NULL,  
  created\_at timestamp without time zone NOT NULL DEFAULT now(),  
  updated\_at timestamp without time zone NOT NULL DEFAULT now(),  
  CONSTRAINT one\_time\_tokens\_pkey PRIMARY KEY (id),  
  CONSTRAINT one\_time\_tokens\_user\_id\_fkey FOREIGN KEY (user\_id) REFERENCES auth.users(id)  
);  
CREATE TABLE auth.refresh\_tokens (  
  instance\_id uuid,  
  id bigint NOT NULL DEFAULT nextval('auth.refresh\_tokens\_id\_seq'::regclass),  
  token character varying UNIQUE,  
  user\_id character varying,  
  revoked boolean,  
  created\_at timestamp with time zone,  
  updated\_at timestamp with time zone,  
  parent character varying,  
  session\_id uuid,  
  CONSTRAINT refresh\_tokens\_pkey PRIMARY KEY (id),  
  CONSTRAINT refresh\_tokens\_session\_id\_fkey FOREIGN KEY (session\_id) REFERENCES auth.sessions(id)  
);  
CREATE TABLE auth.saml\_providers (  
  id uuid NOT NULL,  
  sso\_provider\_id uuid NOT NULL,  
  entity\_id text NOT NULL UNIQUE CHECK (char\_length(entity\_id) \> 0),  
  metadata\_xml text NOT NULL CHECK (char\_length(metadata\_xml) \> 0),  
  metadata\_url text CHECK (metadata\_url \= NULL::text OR char\_length(metadata\_url) \> 0),  
  attribute\_mapping jsonb,  
  created\_at timestamp with time zone,  
  updated\_at timestamp with time zone,  
  name\_id\_format text,  
  CONSTRAINT saml\_providers\_pkey PRIMARY KEY (id),  
  CONSTRAINT saml\_providers\_sso\_provider\_id\_fkey FOREIGN KEY (sso\_provider\_id) REFERENCES auth.sso\_providers(id)  
);  
CREATE TABLE auth.saml\_relay\_states (  
  id uuid NOT NULL,  
  sso\_provider\_id uuid NOT NULL,  
  request\_id text NOT NULL CHECK (char\_length(request\_id) \> 0),  
  for\_email text,  
  redirect\_to text,  
  created\_at timestamp with time zone,  
  updated\_at timestamp with time zone,  
  flow\_state\_id uuid,  
  CONSTRAINT saml\_relay\_states\_pkey PRIMARY KEY (id),  
  CONSTRAINT saml\_relay\_states\_sso\_provider\_id\_fkey FOREIGN KEY (sso\_provider\_id) REFERENCES auth.sso\_providers(id),  
  CONSTRAINT saml\_relay\_states\_flow\_state\_id\_fkey FOREIGN KEY (flow\_state\_id) REFERENCES auth.flow\_state(id)  
);  
CREATE TABLE auth.schema\_migrations (  
  version character varying NOT NULL,  
  CONSTRAINT schema\_migrations\_pkey PRIMARY KEY (version)  
);  
CREATE TABLE auth.sessions (  
  ...
  factor\_id uuid,  
  aal USER-DEFINED,  
  not\_after timestamp with time zone,  
  refreshed\_at timestamp without time zone,  
  user\_agent text,  
  ip inet,  
  tag text,  
  oauth\_client\_id uuid,  
  refresh\_token\_hmac\_key text,  
  refresh\_token\_counter bigint,  
  scopes text CHECK (char\_length(scopes) \<= 4096),  
  CONSTRAINT sessions\_pkey PRIMARY KEY (id),  
  CONSTRAINT sessions\_user\_id\_fkey FOREIGN KEY (user\_id) REFERENCES auth.users(id),  
  CONSTRAINT sessions\_oauth\_client\_id\_fkey FOREIGN KEY (oauth\_client\_id) REFERENCES auth.oauth\_clients(id)  
);  
CREATE TABLE auth.sso\_domains (  
  id uuid NOT NULL,  
  sso\_provider\_id uuid NOT NULL,  
  domain text NOT NULL CHECK (char\_length(domain) \> 0),  
  created\_at timestamp with time zone,  
  updated\_at timestamp with time zone,  
  CONSTRAINT sso\_domains\_pkey PRIMARY KEY (id),  
  CONSTRAINT sso\_domains\_sso\_provider\_id\_fkey FOREIGN KEY (sso\_provider\_id) REFERENCES auth.sso\_providers(id)  
);  
CREATE TABLE auth.sso\_providers (  
  id uuid NOT NULL,  
  resource\_id text CHECK (resource\_id \= NULL::text OR char\_length(resource\_id) \> 0),  
  created\_at timestamp with time zone,  
  updated\_at timestamp with time zone,  
  disabled boolean,  
  CONSTRAINT sso\_providers\_pkey PRIMARY KEY (id)  
);  
CREATE TABLE auth.users (  
  ...
  email\_confirmed\_at timestamp with time zone,  
  invited\_at timestamp with time zone,  
  confirmation\_token character varying,  
  confirmation\_sent\_at timestamp with time zone,  
  recovery\_token character varying,  
  recovery\_sent\_at timestamp with time zone,  
  email\_change\_token\_new character varying,  
  email\_change character varying,  
  email\_change\_sent\_at timestamp with time zone,  
  last\_sign\_in\_at timestamp with time zone,  
  raw\_app\_meta\_data jsonb,  
  raw\_user\_meta\_data jsonb,  
  is\_super\_admin boolean,  
  created\_at timestamp with time zone,  
  updated\_at timestamp with time zone,  
  phone text DEFAULT NULL::character varying UNIQUE,  
  phone\_confirmed\_at timestamp with time zone,  
  phone\_change text DEFAULT ''::character varying,  
  phone\_change\_token character varying DEFAULT ''::character varying,  
  phone\_change\_sent\_at timestamp with time zone,  
  confirmed\_at timestamp with time zone DEFAULT LEAST(email\_confirmed\_at, phone\_confirmed\_at),  
  email\_change\_token\_current character varying DEFAULT ''::character varying,  
  email\_change\_confirm\_status smallint DEFAULT 0 CHECK (email\_change\_confirm\_status \>= 0 AND email\_change\_confirm\_status \<= 2),  
  banned\_until timestamp with time zone,  
  reauthentication\_token character varying DEFAULT ''::character varying,  
  reauthentication\_sent\_at timestamp with time zone,  
  is\_sso\_user boolean NOT NULL DEFAULT false,  
  deleted\_at timestamp with time zone,  
  is\_anonymous boolean NOT NULL DEFAULT false,  
  CONSTRAINT users\_pkey PRIMARY KEY (id)  
);
...
CREATE TABLE realtime.messages (  
  ...
  updated\_at timestamp without time zone NOT NULL DEFAULT now(),  
  inserted\_at timestamp without time zone NOT NULL DEFAULT now(),  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  CONSTRAINT messages\_pkey PRIMARY KEY (id, inserted\_at)  
);  
CREATE TABLE realtime.messages\_2026\_01\_30 (  
  topic text NOT NULL,  
  extension text NOT NULL,  
  payload jsonb,  
  event text,  
  ...
  inserted\_at timestamp without time zone NOT NULL DEFAULT now(),  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  CONSTRAINT messages\_2026\_01\_30\_pkey PRIMARY KEY (id, inserted\_at)  
);  
CREATE TABLE realtime.messages\_2026\_01\_31 (  
  topic text NOT NULL,  
  extension text NOT NULL,  
  payload jsonb,  
  event text,  
  ...
  inserted\_at timestamp without time zone NOT NULL DEFAULT now(),  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  CONSTRAINT messages\_2026\_01\_31\_pkey PRIMARY KEY (id, inserted\_at)  
);  
CREATE TABLE realtime.messages\_2026\_02\_01 (  
  topic text NOT NULL,  
  extension text NOT NULL,  
  payload jsonb,  
  event text,  
  ...
  inserted\_at timestamp without time zone NOT NULL DEFAULT now(),  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  CONSTRAINT messages\_2026\_02\_01\_pkey PRIMARY KEY (id, inserted\_at)  
);  
CREATE TABLE realtime.messages\_2026\_02\_02 (  
  topic text NOT NULL,  
  extension text NOT NULL,  
  payload jsonb,  
  event text,  
  ...
  inserted\_at timestamp without time zone NOT NULL DEFAULT now(),  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  CONSTRAINT messages\_2026\_02\_02\_pkey PRIMARY KEY (id, inserted\_at)  
);  
CREATE TABLE realtime.messages\_2026\_02\_03 (  
  topic text NOT NULL,  
  extension text NOT NULL,  
  payload jsonb,  
  event text,  
  ...
  inserted\_at timestamp without time zone NOT NULL DEFAULT now(),  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  CONSTRAINT messages\_2026\_02\_03\_pkey PRIMARY KEY (id, inserted\_at)  
);  
CREATE TABLE realtime.messages\_2026\_02\_04 (  
  topic text NOT NULL,  
  extension text NOT NULL,  
  payload jsonb,  
  event text,  
  ...
  inserted\_at timestamp without time zone NOT NULL DEFAULT now(),  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  CONSTRAINT messages\_2026\_02\_04\_pkey PRIMARY KEY (id, inserted\_at)  
);  
CREATE TABLE realtime.messages\_2026\_02\_05 (  
  topic text NOT NULL,  
  extension text NOT NULL,  
  payload jsonb,  
  event text,  
  ...
  inserted\_at timestamp without time zone NOT NULL DEFAULT now(),  
  id uuid NOT NULL DEFAULT gen\_random\_uuid(),  
  CONSTRAINT messages\_2026\_02\_05\_pkey PRIMARY KEY (id, inserted\_at)  
);  
CREATE TABLE realtime.schema\_migrations (  
  version bigint NOT NULL,  
  inserted\_at timestamp without time zone,  
  CONSTRAINT schema\_migrations\_pkey PRIMARY KEY (version)  
);  
CREATE TABLE realtime.subscription (  
  id bigint GENERATED ALWAYS AS IDENTITY NOT NULL,  
  subscription\_id uuid NOT NULL,  
  entity regclass NOT NULL,  
  filters ARRAY NOT NULL DEFAULT '{}'::realtime.user\_defined\_filter\[\],  
  claims jsonb NOT NULL,  
  claims\_role regrole NOT NULL DEFAULT realtime.to\_regrole((claims \-\>\> 'role'::text)),  
  created\_at timestamp without time zone NOT NULL DEFAULT timezone('utc'::text, now()),  
  CONSTRAINT subscription\_pkey PRIMARY KEY (id)  
);
...
THE FOLLOWING IS IN CSV FORMAT AND CONTAINS ALL INDEXES ACROSS PUBLIC, AUTH, REALTIME AND UTIL SCHEMAS:

schema\_name,table\_name,index\_name,is\_primary,is\_unique,is\_live,is\_valid,is\_clustered,predicate,comment,indexdef  
auth,audit\_log\_entries,audit\_log\_entries\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX audit\_log\_entries\_pkey ON auth.audit\_log\_entries USING btree (id)  
auth,audit\_log\_entries,audit\_logs\_instance\_id\_idx,false,false,true,true,false,null,null,CREATE INDEX audit\_logs\_instance\_id\_idx ON auth.audit\_log\_entries USING btree (instance\_id)  
auth,flow\_state,flow\_state\_created\_at\_idx,false,false,true,true,false,null,null,CREATE INDEX flow\_state\_created\_at\_idx ON auth.flow\_state USING btree (created\_at DESC)  
auth,flow\_state,flow\_state\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX flow\_state\_pkey ON auth.flow\_state USING btree (id)  
auth,flow\_state,idx\_auth\_code,false,false,true,true,false,null,null,CREATE INDEX idx\_auth\_code ON auth.flow\_state USING btree (auth\_code)  
auth,flow\_state,idx\_user\_id\_auth\_method,false,false,true,true,false,null,null,"CREATE INDEX idx\_user\_id\_auth\_method ON auth.flow\_state USING btree (user\_id, authentication\_method)"  
auth,identities,identities\_email\_idx,false,false,true,true,false,null,Auth: Ensures indexed queries on the email column,CREATE INDEX identities\_email\_idx ON auth.identities USING btree (email text\_pattern\_ops)  
auth,identities,identities\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX identities\_pkey ON auth.identities USING btree (id)  
auth,identities,identities\_provider\_id\_provider\_unique,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX identities\_provider\_id\_provider\_unique ON auth.identities USING btree (provider\_id, provider)"  
auth,identities,identities\_user\_id\_idx,false,false,true,true,false,null,null,CREATE INDEX identities\_user\_id\_idx ON auth.identities USING btree (user\_id)  
auth,instances,instances\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX instances\_pkey ON auth.instances USING btree (id)  
auth,mfa\_amr\_claims,amr\_id\_pk,true,true,true,true,false,null,null,CREATE UNIQUE INDEX amr\_id\_pk ON auth.mfa\_amr\_claims USING btree (id)  
auth,mfa\_amr\_claims,mfa\_amr\_claims\_session\_id\_authentication\_method\_pkey,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX mfa\_amr\_claims\_session\_id\_authentication\_method\_pkey ON auth.mfa\_amr\_claims USING btree (session\_id, authentication\_method)"  
auth,mfa\_challenges,mfa\_challenge\_created\_at\_idx,false,false,true,true,false,null,null,CREATE INDEX mfa\_challenge\_created\_at\_idx ON auth.mfa\_challenges USING btree (created\_at DESC)  
auth,mfa\_challenges,mfa\_challenges\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX mfa\_challenges\_pkey ON auth.mfa\_challenges USING btree (id)  
auth,mfa\_factors,factor\_id\_created\_at\_idx,false,false,true,true,false,null,null,"CREATE INDEX factor\_id\_created\_at\_idx ON auth.mfa\_factors USING btree (user\_id, created\_at)"  
auth,mfa\_factors,mfa\_factors\_last\_challenged\_at\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX mfa\_factors\_last\_challenged\_at\_key ON auth.mfa\_factors USING btree (last\_challenged\_at)  
auth,mfa\_factors,mfa\_factors\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX mfa\_factors\_pkey ON auth.mfa\_factors USING btree (id)  
auth,mfa\_factors,mfa\_factors\_user\_friendly\_name\_unique,false,true,true,true,false,(TRIM(BOTH FROM friendly\_name) \<\> ''::text),null,"CREATE UNIQUE INDEX mfa\_factors\_user\_friendly\_name\_unique ON auth.mfa\_factors USING btree (friendly\_name, user\_id) WHERE (TRIM(BOTH FROM friendly\_name) \<\> ''::text)"  
auth,mfa\_factors,mfa\_factors\_user\_id\_idx,false,false,true,true,false,null,null,CREATE INDEX mfa\_factors\_user\_id\_idx ON auth.mfa\_factors USING btree (user\_id)  
auth,mfa\_factors,unique\_phone\_factor\_per\_user,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX unique\_phone\_factor\_per\_user ON auth.mfa\_factors USING btree (user\_id, phone)"  
auth,oauth\_authorizations,oauth\_auth\_pending\_exp\_idx,false,false,true,true,false,(status \= 'pending'::auth.oauth\_authorization\_status),null,CREATE INDEX oauth\_auth\_pending\_exp\_idx ON auth.oauth\_authorizations USING btree (expires\_at) WHERE (status \= 'pending'::auth.oauth\_authorization\_status)  
auth,oauth\_authorizations,oauth\_authorizations\_authorization\_code\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX oauth\_authorizations\_authorization\_code\_key ON auth.oauth\_authorizations USING btree (authorization\_code)  
auth,oauth\_authorizations,oauth\_authorizations\_authorization\_id\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX oauth\_authorizations\_authorization\_id\_key ON auth.oauth\_authorizations USING btree (authorization\_id)  
auth,oauth\_authorizations,oauth\_authorizations\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX oauth\_authorizations\_pkey ON auth.oauth\_authorizations USING btree (id)  
auth,oauth\_client\_states,idx\_oauth\_client\_states\_created\_at,false,false,true,true,false,null,null,CREATE INDEX idx\_oauth\_client\_states\_created\_at ON auth.oauth\_client\_states USING btree (created\_at)  
auth,oauth\_client\_states,oauth\_client\_states\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX oauth\_client\_states\_pkey ON auth.oauth\_client\_states USING btree (id)  
auth,oauth\_clients,oauth\_clients\_deleted\_at\_idx,false,false,true,true,false,null,null,CREATE INDEX oauth\_clients\_deleted\_at\_idx ON auth.oauth\_clients USING btree (deleted\_at)  
auth,oauth\_clients,oauth\_clients\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX oauth\_clients\_pkey ON auth.oauth\_clients USING btree (id)  
auth,oauth\_consents,oauth\_consents\_active\_client\_idx,false,false,true,true,false,(revoked\_at IS NULL),null,CREATE INDEX oauth\_consents\_active\_client\_idx ON auth.oauth\_consents USING btree (client\_id) WHERE (revoked\_at IS NULL)  
auth,oauth\_consents,oauth\_consents\_active\_user\_client\_idx,false,false,true,true,false,(revoked\_at IS NULL),null,"CREATE INDEX oauth\_consents\_active\_user\_client\_idx ON auth.oauth\_consents USING btree (user\_id, client\_id) WHERE (revoked\_at IS NULL)"  
auth,oauth\_consents,oauth\_consents\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX oauth\_consents\_pkey ON auth.oauth\_consents USING btree (id)  
auth,oauth\_consents,oauth\_consents\_user\_client\_unique,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX oauth\_consents\_user\_client\_unique ON auth.oauth\_consents USING btree (user\_id, client\_id)"  
auth,oauth\_consents,oauth\_consents\_user\_order\_idx,false,false,true,true,false,null,null,"CREATE INDEX oauth\_consents\_user\_order\_idx ON auth.oauth\_consents USING btree (user\_id, granted\_at DESC)"  
auth,one\_time\_tokens,one\_time\_tokens\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX one\_time\_tokens\_pkey ON auth.one\_time\_tokens USING btree (id)  
auth,one\_time\_tokens,one\_time\_tokens\_relates\_to\_hash\_idx,false,false,true,true,false,null,null,CREATE INDEX one\_time\_tokens\_relates\_to\_hash\_idx ON auth.one\_time\_tokens USING hash (relates\_to)  
auth,one\_time\_tokens,one\_time\_tokens\_token\_hash\_hash\_idx,false,false,true,true,false,null,null,CREATE INDEX one\_time\_tokens\_token\_hash\_hash\_idx ON auth.one\_time\_tokens USING hash (token\_hash)  
auth,one\_time\_tokens,one\_time\_tokens\_user\_id\_token\_type\_key,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX one\_time\_tokens\_user\_id\_token\_type\_key ON auth.one\_time\_tokens USING btree (user\_id, token\_type)"  
auth,refresh\_tokens,refresh\_tokens\_instance\_id\_idx,false,false,true,true,false,null,null,CREATE INDEX refresh\_tokens\_instance\_id\_idx ON auth.refresh\_tokens USING btree (instance\_id)  
auth,refresh\_tokens,refresh\_tokens\_instance\_id\_user\_id\_idx,false,false,true,true,false,null,null,"CREATE INDEX refresh\_tokens\_instance\_id\_user\_id\_idx ON auth.refresh\_tokens USING btree (instance\_id, user\_id)"  
auth,refresh\_tokens,refresh\_tokens\_parent\_idx,false,false,true,true,false,null,null,CREATE INDEX refresh\_tokens\_parent\_idx ON auth.refresh\_tokens USING btree (parent)  
auth,refresh\_tokens,refresh\_tokens\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX refresh\_tokens\_pkey ON auth.refresh\_tokens USING btree (id)  
auth,refresh\_tokens,refresh\_tokens\_session\_id\_revoked\_idx,false,false,true,true,false,null,null,"CREATE INDEX refresh\_tokens\_session\_id\_revoked\_idx ON auth.refresh\_tokens USING btree (session\_id, revoked)"  
auth,refresh\_tokens,refresh\_tokens\_token\_unique,false,true,true,true,false,null,null,CREATE UNIQUE INDEX refresh\_tokens\_token\_unique ON auth.refresh\_tokens USING btree (token)  
auth,refresh\_tokens,refresh\_tokens\_updated\_at\_idx,false,false,true,true,false,null,null,CREATE INDEX refresh\_tokens\_updated\_at\_idx ON auth.refresh\_tokens USING btree (updated\_at DESC)  
auth,saml\_providers,saml\_providers\_entity\_id\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX saml\_providers\_entity\_id\_key ON auth.saml\_providers USING btree (entity\_id)  
auth,saml\_providers,saml\_providers\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX saml\_providers\_pkey ON auth.saml\_providers USING btree (id)  
auth,saml\_providers,saml\_providers\_sso\_provider\_id\_idx,false,false,true,true,false,null,null,CREATE INDEX saml\_providers\_sso\_provider\_id\_idx ON auth.saml\_providers USING btree (sso\_provider\_id)  
auth,saml\_relay\_states,saml\_relay\_states\_created\_at\_idx,false,false,true,true,false,null,null,CREATE INDEX saml\_relay\_states\_created\_at\_idx ON auth.saml\_relay\_states USING btree (created\_at DESC)  
auth,saml\_relay\_states,saml\_relay\_states\_for\_email\_idx,false,false,true,true,false,null,null,CREATE INDEX saml\_relay\_states\_for\_email\_idx ON auth.saml\_relay\_states USING btree (for\_email)  
auth,saml\_relay\_states,saml\_relay\_states\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX saml\_relay\_states\_pkey ON auth.saml\_relay\_states USING btree (id)  
auth,saml\_relay\_states,saml\_relay\_states\_sso\_provider\_id\_idx,false,false,true,true,false,null,null,CREATE INDEX saml\_relay\_states\_sso\_provider\_id\_idx ON auth.saml\_relay\_states USING btree (sso\_provider\_id)  
auth,schema\_migrations,schema\_migrations\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX schema\_migrations\_pkey ON auth.schema\_migrations USING btree (version)  
auth,sessions,sessions\_not\_after\_idx,false,false,true,true,false,null,null,CREATE INDEX sessions\_not\_after\_idx ON auth.sessions USING btree (not\_after DESC)  
auth,sessions,sessions\_oauth\_client\_id\_idx,false,false,true,true,false,null,null,CREATE INDEX sessions\_oauth\_client\_id\_idx ON auth.sessions USING btree (oauth\_client\_id)  
auth,sessions,sessions\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX sessions\_pkey ON auth.sessions USING btree (id)  
auth,sessions,sessions\_user\_id\_idx,false,false,true,true,false,null,null,CREATE INDEX sessions\_user\_id\_idx ON auth.sessions USING btree (user\_id)  
auth,sessions,user\_id\_created\_at\_idx,false,false,true,true,false,null,null,"CREATE INDEX user\_id\_created\_at\_idx ON auth.sessions USING btree (user\_id, created\_at)"  
auth,sso\_domains,sso\_domains\_domain\_idx,false,true,true,true,false,null,null,CREATE UNIQUE INDEX sso\_domains\_domain\_idx ON auth.sso\_domains USING btree (lower(domain))  
auth,sso\_domains,sso\_domains\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX sso\_domains\_pkey ON auth.sso\_domains USING btree (id)  
auth,sso\_domains,sso\_domains\_sso\_provider\_id\_idx,false,false,true,true,false,null,null,CREATE INDEX sso\_domains\_sso\_provider\_id\_idx ON auth.sso\_domains USING btree (sso\_provider\_id)  
auth,sso\_providers,sso\_providers\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX sso\_providers\_pkey ON auth.sso\_providers USING btree (id)  
auth,sso\_providers,sso\_providers\_resource\_id\_idx,false,true,true,true,false,null,null,CREATE UNIQUE INDEX sso\_providers\_resource\_id\_idx ON auth.sso\_providers USING btree (lower(resource\_id))  
auth,sso\_providers,sso\_providers\_resource\_id\_pattern\_idx,false,false,true,true,false,null,null,CREATE INDEX sso\_providers\_resource\_id\_pattern\_idx ON auth.sso\_providers USING btree (resource\_id text\_pattern\_ops)  
auth,users,confirmation\_token\_idx,false,true,true,true,false,((confirmation\_token)::text \!\~ '^\[0-9 \]\*$'::text),null,CREATE UNIQUE INDEX confirmation\_token\_idx ON auth.users USING btree (confirmation\_token) WHERE ((confirmation\_token)::text \!\~ '^\[0-9 \]\*$'::text)  
auth,users,email\_change\_token\_current\_idx,false,true,true,true,false,((email\_change\_token\_current)::text \!\~ '^\[0-9 \]\*$'::text),null,CREATE UNIQUE INDEX email\_change\_token\_current\_idx ON auth.users USING btree (email\_change\_token\_current) WHERE ((email\_change\_token\_current)::text \!\~ '^\[0-9 \]\*$'::text)  
auth,users,email\_change\_token\_new\_idx,false,true,true,true,false,((email\_change\_token\_new)::text \!\~ '^\[0-9 \]\*$'::text),null,CREATE UNIQUE INDEX email\_change\_token\_new\_idx ON auth.users USING btree (email\_change\_token\_new) WHERE ((email\_change\_token\_new)::text \!\~ '^\[0-9 \]\*$'::text)  
auth,users,reauthentication\_token\_idx,false,true,true,true,false,((reauthentication\_token)::text \!\~ '^\[0-9 \]\*$'::text),null,CREATE UNIQUE INDEX reauthentication\_token\_idx ON auth.users USING btree (reauthentication\_token) WHERE ((reauthentication\_token)::text \!\~ '^\[0-9 \]\*$'::text)  
auth,users,recovery\_token\_idx,false,true,true,true,false,((recovery\_token)::text \!\~ '^\[0-9 \]\*$'::text),null,CREATE UNIQUE INDEX recovery\_token\_idx ON auth.users USING btree (recovery\_token) WHERE ((recovery\_token)::text \!\~ '^\[0-9 \]\*$'::text)  
auth,users,users\_email\_partial\_key,false,true,true,true,false,(is\_sso\_user \= false),Auth: A partial unique index that applies only when is\_sso\_user is false,CREATE UNIQUE INDEX users\_email\_partial\_key ON auth.users USING btree (email) WHERE (is\_sso\_user \= false)  
auth,users,users\_instance\_id\_email\_idx,false,false,true,true,false,null,null,"CREATE INDEX users\_instance\_id\_email\_idx ON auth.users USING btree (instance\_id, lower((email)::text))"  
auth,users,users\_instance\_id\_idx,false,false,true,true,false,null,null,CREATE INDEX users\_instance\_id\_idx ON auth.users USING btree (instance\_id)  
auth,users,users\_is\_anonymous\_idx,false,false,true,true,false,null,null,CREATE INDEX users\_is\_anonymous\_idx ON auth.users USING btree (is\_anonymous)  
auth,users,users\_phone\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX users\_phone\_key ON auth.users USING btree (phone)  
auth,users,users\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX users\_pkey ON auth.users USING btree (id)  
public,Prize\_Instantprizes,Prize\_Instantprizes\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX ""Prize\_Instantprizes\_pkey"" ON public.""Prize\_Instantprizes"" USING btree (""UID"")"  
public,Prize\_Instantprizes,idx\_instant\_prizes\_winning\_ticket,false,false,true,true,false,null,null,"CREATE INDEX idx\_instant\_prizes\_winning\_ticket ON public.""Prize\_Instantprizes"" USING btree (""competitionId"", ""winningTicket"")"  
public,\_entries\_progress,\_entries\_progress\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX \_entries\_progress\_pkey ON public.\_entries\_progress USING btree (competition\_id, canonical\_user\_id)"  
public,\_payment\_settings,\_payment\_settings\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX \_payment\_settings\_pkey ON public.\_payment\_settings USING btree (key)  
public,admin\_sessions,admin\_sessions\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX admin\_sessions\_pkey ON public.admin\_sessions USING btree (id)  
public,admin\_sessions,admin\_sessions\_token\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX admin\_sessions\_token\_key ON public.admin\_sessions USING btree (token)  
public,admin\_users,admin\_users\_email\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX admin\_users\_email\_key ON public.admin\_users USING btree (email)  
public,admin\_users,admin\_users\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX admin\_users\_pkey ON public.admin\_users USING btree (id)  
public,admin\_users\_audit,admin\_users\_audit\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX admin\_users\_audit\_pkey ON public.admin\_users\_audit USING btree (id)  
public,balance\_ledger,balance\_ledger\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX balance\_ledger\_pkey ON public.balance\_ledger USING btree (id)  
public,balance\_ledger,balance\_ledger\_reference\_unique,false,true,true,true,false,null,null,CREATE UNIQUE INDEX balance\_ledger\_reference\_unique ON public.balance\_ledger USING btree (reference\_id)  
public,balance\_ledger,idx\_balance\_ledger\_canonical,false,false,true,true,false,null,null,CREATE INDEX idx\_balance\_ledger\_canonical ON public.balance\_ledger USING btree (canonical\_user\_id)  
public,balance\_ledger,idx\_balance\_ledger\_user\_created,false,false,true,true,false,null,null,"CREATE INDEX idx\_balance\_ledger\_user\_created ON public.balance\_ledger USING btree (canonical\_user\_id, created\_at DESC)"  
public,balance\_ledger,idx\_ledger\_user\_created,false,false,true,true,false,null,null,"CREATE INDEX idx\_ledger\_user\_created ON public.balance\_ledger USING btree (canonical\_user\_id, created\_at)"  
public,balance\_ledger,u\_balance\_ledger\_reference\_id,false,true,true,true,false,null,null,CREATE UNIQUE INDEX u\_balance\_ledger\_reference\_id ON public.balance\_ledger USING btree (reference\_id)  
public,bonus\_award\_audit,bonus\_award\_audit\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX bonus\_award\_audit\_pkey ON public.bonus\_award\_audit USING btree (id)  
public,canonical\_users,canonical\_users\_base\_wallet\_address\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX canonical\_users\_base\_wallet\_address\_key ON public.canonical\_users USING btree (base\_wallet\_address)  
public,canonical\_users,canonical\_users\_canonical\_user\_id\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX canonical\_users\_canonical\_user\_id\_key ON public.canonical\_users USING btree (canonical\_user\_id)  
public,canonical\_users,canonical\_users\_email\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX canonical\_users\_email\_key ON public.canonical\_users USING btree (email)  
public,canonical\_users,canonical\_users\_eth\_wallet\_address\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX canonical\_users\_eth\_wallet\_address\_key ON public.canonical\_users USING btree (eth\_wallet\_address)  
public,canonical\_users,canonical\_users\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX canonical\_users\_pkey ON public.canonical\_users USING btree (id)  
public,canonical\_users,canonical\_users\_uid\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX canonical\_users\_uid\_key ON public.canonical\_users USING btree (uid)  
public,canonical\_users,canonical\_users\_wallet\_address\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX canonical\_users\_wallet\_address\_key ON public.canonical\_users USING btree (wallet\_address)  
public,canonical\_users,idx\_canonical\_users\_canonical\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_canonical\_users\_canonical\_id ON public.canonical\_users USING btree (canonical\_user\_id)  
public,canonical\_users,idx\_canonical\_users\_privy\_user\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_canonical\_users\_privy\_user\_id ON public.canonical\_users USING btree (privy\_user\_id)  
public,canonical\_users,idx\_canonical\_users\_wallet,false,false,true,true,false,null,null,CREATE INDEX idx\_canonical\_users\_wallet ON public.canonical\_users USING btree (wallet\_address)  
public,canonical\_users,idx\_cu\_wallet,false,false,true,true,false,null,null,CREATE INDEX idx\_cu\_wallet ON public.canonical\_users USING btree (wallet\_address)  
public,canonical\_users,uq\_canonical\_users\_canonical,false,true,true,true,false,(canonical\_user\_id IS NOT NULL),null,CREATE UNIQUE INDEX uq\_canonical\_users\_canonical ON public.canonical\_users USING btree (canonical\_user\_id) WHERE (canonical\_user\_id IS NOT NULL)  
public,cdp\_event\_queue,cdp\_event\_queue\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX cdp\_event\_queue\_pkey ON public.cdp\_event\_queue USING btree (id)  
public,competition\_entries,competition\_entries\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX competition\_entries\_pkey ON public.competition\_entries USING btree (id)  
public,competition\_entries,competition\_entries\_unique,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX competition\_entries\_unique ON public.competition\_entries USING btree (canonical\_user\_id, competition\_id)"  
public,competition\_entries,idx\_comp\_entries\_user\_comp,false,false,true,true,false,null,null,"CREATE INDEX idx\_comp\_entries\_user\_comp ON public.competition\_entries USING btree (canonical\_user\_id, competition\_id)"  
public,competition\_entries,idx\_competition\_entries\_unique,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX idx\_competition\_entries\_unique ON public.competition\_entries USING btree (competition\_id, canonical\_user\_id)"  
public,competition\_entries,idx\_competition\_entries\_user\_comp,false,false,true,true,false,null,null,"CREATE INDEX idx\_competition\_entries\_user\_comp ON public.competition\_entries USING btree (canonical\_user\_id, competition\_id)"  
public,competition\_entries,ux\_comp\_entries\_comp\_user,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX ux\_comp\_entries\_comp\_user ON public.competition\_entries USING btree (competition\_id, canonical\_user\_id)"  
public,competition\_entries,ux\_competition\_entries\_comp\_user,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX ux\_competition\_entries\_comp\_user ON public.competition\_entries USING btree (competition\_id, canonical\_user\_id)"  
public,competitions,competitions\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX competitions\_pkey ON public.competitions USING btree (id)  
public,competitions,idx\_competitions\_end\_date,false,false,true,true,false,null,null,CREATE INDEX idx\_competitions\_end\_date ON public.competitions USING btree (end\_date)  
public,competitions,idx\_competitions\_status,false,false,true,true,false,null,null,CREATE INDEX idx\_competitions\_status ON public.competitions USING btree (status)  
public,competitions,idx\_competitions\_status\_dates,false,false,true,true,false,null,null,"CREATE INDEX idx\_competitions\_status\_dates ON public.competitions USING btree (status, start\_time, end\_time)"  
public,competitions,idx\_competitions\_vrf\_completed\_at,false,false,true,true,false,null,null,CREATE INDEX idx\_competitions\_vrf\_completed\_at ON public.competitions USING btree (vrf\_completed\_at)  
public,competitions,idx\_competitions\_vrf\_draw\_requested\_at,false,false,true,true,false,null,null,CREATE INDEX idx\_competitions\_vrf\_draw\_requested\_at ON public.competitions USING btree (vrf\_draw\_requested\_at)  
public,confirmation\_incident\_log,confirmation\_incident\_log\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX confirmation\_incident\_log\_pkey ON public.confirmation\_incident\_log USING btree (id)  
public,custody\_transactions,custody\_transactions\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX custody\_transactions\_pkey ON public.custody\_transactions USING btree (id)  
public,custody\_transactions,idx\_custody\_transactions\_canonical,false,false,true,true,false,null,null,CREATE INDEX idx\_custody\_transactions\_canonical ON public.custody\_transactions USING btree (canonical\_user\_id)  
public,custody\_transactions,idx\_custody\_transactions\_status,false,false,true,true,false,null,null,CREATE INDEX idx\_custody\_transactions\_status ON public.custody\_transactions USING btree (status)  
public,custody\_transactions,idx\_custody\_tx\_user\_status\_type\_created,false,false,true,true,false,null,null,"CREATE INDEX idx\_custody\_tx\_user\_status\_type\_created ON public.custody\_transactions USING btree (user\_id, status, transaction\_type, created\_at)"  
public,email\_auth\_sessions,email\_auth\_sessions\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX email\_auth\_sessions\_pkey ON public.email\_auth\_sessions USING btree (id)  
public,email\_auth\_sessions,idx\_email\_auth\_sessions\_email,false,false,true,true,false,null,null,CREATE INDEX idx\_email\_auth\_sessions\_email ON public.email\_auth\_sessions USING btree (email)  
public,enqueue\_cdp\_event,cdp\_outbox\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX cdp\_outbox\_pkey ON public.enqueue\_cdp\_event USING btree (id)  
public,faqs,faqs\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX faqs\_pkey ON public.faqs USING btree (id)  
public,hero\_competitions,hero\_competitions\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX hero\_competitions\_pkey ON public.hero\_competitions USING btree (id)  
public,hero\_competitions,idx\_hero\_competitions\_competition\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_hero\_competitions\_competition\_id ON public.hero\_competitions USING btree (competition\_id)  
public,instant\_win\_grids,idx\_instant\_win\_grids\_competition\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_instant\_win\_grids\_competition\_id ON public.instant\_win\_grids USING btree (competition\_id)  
public,instant\_win\_grids,idx\_instant\_win\_grids\_is\_active,false,false,true,true,false,(is\_active \= true),null,CREATE INDEX idx\_instant\_win\_grids\_is\_active ON public.instant\_win\_grids USING btree (is\_active) WHERE (is\_active \= true)  
public,instant\_win\_grids,instant\_win\_grids\_competition\_id\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX instant\_win\_grids\_competition\_id\_key ON public.instant\_win\_grids USING btree (competition\_id)  
public,instant\_win\_grids,instant\_win\_grids\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX instant\_win\_grids\_pkey ON public.instant\_win\_grids USING btree (id)  
public,internal\_transfers,internal\_transfers\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX internal\_transfers\_pkey ON public.internal\_transfers USING btree (id)  
public,internal\_transfers,internal\_transfers\_transfer\_id\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX internal\_transfers\_transfer\_id\_key ON public.internal\_transfers USING btree (transfer\_id)  
public,joincompetition,idx\_joincompetition\_canonical\_user\_id,false,false,true,true,false,(canonical\_user\_id IS NOT NULL),null,CREATE INDEX idx\_joincompetition\_canonical\_user\_id ON public.joincompetition USING btree (canonical\_user\_id) WHERE (canonical\_user\_id IS NOT NULL)  
public,joincompetition,idx\_joincompetition\_comp,false,false,true,true,false,null,null,CREATE INDEX idx\_joincompetition\_comp ON public.joincompetition USING btree (competitionid)  
public,joincompetition,idx\_joincompetition\_comp\_user\_wallet,false,false,true,true,false,null,null,"CREATE INDEX idx\_joincompetition\_comp\_user\_wallet ON public.joincompetition USING btree (competitionid, userid, canonical\_user\_id, wallet\_address)"  
public,joincompetition,idx\_joincompetition\_competitionid,false,false,true,true,false,null,null,CREATE INDEX idx\_joincompetition\_competitionid ON public.joincompetition USING btree (competitionid)  
public,joincompetition,idx\_joincompetition\_competitionid\_tickets,false,false,true,true,false,((ticketnumbers IS NOT NULL) AND (TRIM(BOTH FROM ticketnumbers) \<\> ''::text)),null,CREATE INDEX idx\_joincompetition\_competitionid\_tickets ON public.joincompetition USING btree (competitionid) WHERE ((ticketnumbers IS NOT NULL) AND (TRIM(BOTH FROM ticketnumbers) \<\> ''::text))  
public,joincompetition,idx\_joincompetition\_cuid,false,false,true,true,false,null,null,CREATE INDEX idx\_joincompetition\_cuid ON public.joincompetition USING btree (canonical\_user\_id)  
public,joincompetition,idx\_joincompetition\_privy\_user\_id,false,false,true,true,false,(privy\_user\_id IS NOT NULL),null,CREATE INDEX idx\_joincompetition\_privy\_user\_id ON public.joincompetition USING btree (privy\_user\_id) WHERE (privy\_user\_id IS NOT NULL)  
public,joincompetition,idx\_joincompetition\_purchasedate,false,false,true,true,false,null,null,CREATE INDEX idx\_joincompetition\_purchasedate ON public.joincompetition USING btree (purchasedate DESC)  
public,joincompetition,idx\_joincompetition\_transactionhash,false,false,true,true,false,(transactionhash IS NOT NULL),null,CREATE INDEX idx\_joincompetition\_transactionhash ON public.joincompetition USING btree (transactionhash) WHERE (transactionhash IS NOT NULL)  
public,joincompetition,idx\_joincompetition\_user\_comp,false,false,true,true,false,null,null,"CREATE INDEX idx\_joincompetition\_user\_comp ON public.joincompetition USING btree (canonical\_user\_id, competitionid)"  
public,joincompetition,idx\_joincompetition\_userid,false,false,true,true,false,null,null,CREATE INDEX idx\_joincompetition\_userid ON public.joincompetition USING btree (userid)  
public,joincompetition,idx\_joincompetition\_wallet,false,false,true,true,false,null,null,CREATE INDEX idx\_joincompetition\_wallet ON public.joincompetition USING btree (wallet\_address)  
public,joincompetition,idx\_joincompetition\_wallet\_address,false,false,true,true,false,null,null,CREATE INDEX idx\_joincompetition\_wallet\_address ON public.joincompetition USING btree (wallet\_address)  
public,joincompetition,idx\_joincompetition\_walletaddress\_lower,false,false,true,true,false,null,null,CREATE INDEX idx\_joincompetition\_walletaddress\_lower ON public.joincompetition USING btree (lower(wallet\_address))  
public,joincompetition,joincompetition\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX joincompetition\_pkey ON public.joincompetition USING btree (id)  
public,joincompetition,uniq\_joincompetition\_tx\_comp,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX uniq\_joincompetition\_tx\_comp ON public.joincompetition USING btree (transactionhash, competitionid)"  
public,joined\_competitions,idx\_joined\_competitions\_canonical,false,false,true,true,false,null,null,CREATE INDEX idx\_joined\_competitions\_canonical ON public.joined\_competitions USING btree (canonical\_user\_id)  
public,joined\_competitions,idx\_joined\_competitions\_competition\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_joined\_competitions\_competition\_id ON public.joined\_competitions USING btree (competition\_id)  
public,joined\_competitions,joined\_competitions\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX joined\_competitions\_pkey ON public.joined\_competitions USING btree (id)  
public,notifications,idx\_notifications\_cuid,false,false,true,true,false,null,null,CREATE INDEX idx\_notifications\_cuid ON public.notifications USING btree (canonical\_user\_id)  
public,notifications,idx\_notifications\_read,false,false,true,true,false,null,null,"CREATE INDEX idx\_notifications\_read ON public.notifications USING btree (user\_id, read)"  
public,notifications,notifications\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX notifications\_pkey ON public.notifications USING btree (id)  
public,order\_tickets,order\_tickets\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX order\_tickets\_pkey ON public.order\_tickets USING btree (id)  
public,orders,idx\_orders\_canonical\_user\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_orders\_canonical\_user\_id ON public.orders USING btree (canonical\_user\_id)  
public,orders,idx\_orders\_competition\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_orders\_competition\_id ON public.orders USING btree (competition\_id)  
public,orders,orders\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX orders\_pkey ON public.orders USING btree (id)  
public,orders,orders\_unique\_order\_key\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX orders\_unique\_order\_key\_key ON public.orders USING btree (unique\_order\_key)  
public,participants,idx\_participants\_canonical,false,false,true,true,false,null,null,CREATE INDEX idx\_participants\_canonical ON public.participants USING btree (canonical\_user\_id)  
public,participants,idx\_participants\_competition\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_participants\_competition\_id ON public.participants USING btree (competition\_id)  
public,participants,participants\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX participants\_pkey ON public.participants USING btree (id)  
public,partners,partners\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX partners\_pkey ON public.partners USING btree (id)  
public,payment\_idempotency,idx\_payment\_idempotency\_key,false,false,true,true,false,null,null,CREATE INDEX idx\_payment\_idempotency\_key ON public.payment\_idempotency USING btree (idempotency\_key)  
public,payment\_idempotency,payment\_idempotency\_idempotency\_key\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX payment\_idempotency\_idempotency\_key\_key ON public.payment\_idempotency USING btree (idempotency\_key)  
public,payment\_idempotency,payment\_idempotency\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX payment\_idempotency\_pkey ON public.payment\_idempotency USING btree (id)  
public,payment\_webhook\_events,idx\_payment\_webhook\_events\_created\_at,false,false,true,true,false,null,null,CREATE INDEX idx\_payment\_webhook\_events\_created\_at ON public.payment\_webhook\_events USING btree (created\_at)  
public,payment\_webhook\_events,idx\_pwe\_received\_time,false,false,true,true,false,null,null,CREATE INDEX idx\_pwe\_received\_time ON public.payment\_webhook\_events USING btree (received\_at DESC)  
public,payment\_webhook\_events,payment\_webhook\_events\_event\_id\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX payment\_webhook\_events\_event\_id\_key ON public.payment\_webhook\_events USING btree (event\_id)  
public,payment\_webhook\_events,payment\_webhook\_events\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX payment\_webhook\_events\_pkey ON public.payment\_webhook\_events USING btree (id)  
public,payments,idx\_payments\_idem,false,true,true,true,false,null,null,CREATE UNIQUE INDEX idx\_payments\_idem ON public.payments USING btree (idempotency\_key)  
public,payments,idx\_payments\_reservation,false,false,true,true,false,null,null,CREATE INDEX idx\_payments\_reservation ON public.payments USING btree (reservation\_id)  
public,payments,payments\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX payments\_pkey ON public.payments USING btree (id)  
public,payments\_jobs,payments\_jobs\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX payments\_jobs\_pkey ON public.payments\_jobs USING btree (id)  
public,pending\_ticket\_items,idx\_pending\_items\_competition\_ticket,false,false,true,true,false,null,null,"CREATE INDEX idx\_pending\_items\_competition\_ticket ON public.pending\_ticket\_items USING btree (competition\_id, ticket\_number)"  
public,pending\_ticket\_items,idx\_pending\_items\_pending,false,false,true,true,false,null,null,CREATE INDEX idx\_pending\_items\_pending ON public.pending\_ticket\_items USING btree (pending\_ticket\_id)  
public,pending\_ticket\_items,idx\_pending\_ticket\_items\_header,false,false,true,true,false,null,null,CREATE INDEX idx\_pending\_ticket\_items\_header ON public.pending\_ticket\_items USING btree (pending\_ticket\_id)  
public,pending\_ticket\_items,idx\_pti\_pending\_not\_exp,false,false,true,true,false,null,null,"CREATE INDEX idx\_pti\_pending\_not\_exp ON public.pending\_ticket\_items USING btree (status, expires\_at)"  
public,pending\_ticket\_items,pending\_ticket\_items\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX pending\_ticket\_items\_pkey ON public.pending\_ticket\_items USING btree (id)  
public,pending\_ticket\_items,uq\_pending\_ticket\_items\_active,false,true,true,true,false,(status \= 'pending'::text),null,"CREATE UNIQUE INDEX uq\_pending\_ticket\_items\_active ON public.pending\_ticket\_items USING btree (competition\_id, ticket\_number, expires\_at) WHERE (status \= 'pending'::text)"  
public,pending\_ticket\_items,ux\_pending\_pending\_hold,false,true,true,true,false,(status \= 'pending'::text),null,"CREATE UNIQUE INDEX ux\_pending\_pending\_hold ON public.pending\_ticket\_items USING btree (competition\_id, ticket\_number) WHERE (status \= 'pending'::text)"  
public,pending\_tickets,idx\_pending\_tickets\_active\_partial,false,false,true,true,false,"(status \= ANY (ARRAY\['pending'::text, 'awaiting\_payment'::text\]))",null,"CREATE INDEX idx\_pending\_tickets\_active\_partial ON public.pending\_tickets USING btree (competition\_id, expires\_at) WHERE (status \= ANY (ARRAY\['pending'::text, 'awaiting\_payment'::text\]))"  
public,pending\_tickets,idx\_pending\_tickets\_canonical\_user\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_pending\_tickets\_canonical\_user\_id ON public.pending\_tickets USING btree (canonical\_user\_id)  
public,pending\_tickets,idx\_pending\_tickets\_comp\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_pending\_tickets\_comp\_id ON public.pending\_tickets USING btree (competition\_id)  
public,pending\_tickets,idx\_pending\_tickets\_comp\_status,false,false,true,true,false,null,null,"CREATE INDEX idx\_pending\_tickets\_comp\_status ON public.pending\_tickets USING btree (competition\_id, status)"  
public,pending\_tickets,idx\_pending\_tickets\_comp\_status\_exp,false,false,true,true,false,null,null,"CREATE INDEX idx\_pending\_tickets\_comp\_status\_exp ON public.pending\_tickets USING btree (competition\_id, status, expires\_at)"  
public,pending\_tickets,idx\_pending\_tickets\_comp\_status\_user,false,false,true,true,false,(status \= 'pending'::text),null,"CREATE INDEX idx\_pending\_tickets\_comp\_status\_user ON public.pending\_tickets USING btree (competition\_id, status, user\_id) WHERE (status \= 'pending'::text)"  
public,pending\_tickets,idx\_pending\_tickets\_comp\_user,false,false,true,true,false,null,null,"CREATE INDEX idx\_pending\_tickets\_comp\_user ON public.pending\_tickets USING btree (competition\_id, user\_id, canonical\_user\_id)"  
public,pending\_tickets,idx\_pending\_tickets\_competition,false,false,true,true,false,null,null,CREATE INDEX idx\_pending\_tickets\_competition ON public.pending\_tickets USING btree (competition\_id)  
public,pending\_tickets,idx\_pending\_tickets\_expires,false,false,true,true,false,null,null,CREATE INDEX idx\_pending\_tickets\_expires ON public.pending\_tickets USING btree (expires\_at)  
public,pending\_tickets,idx\_pending\_tickets\_identifiers,false,false,true,true,false,null,null,"CREATE INDEX idx\_pending\_tickets\_identifiers ON public.pending\_tickets USING btree (canonical\_user\_id, user\_id)"  
public,pending\_tickets,idx\_pending\_tickets\_privy,false,false,true,true,false,null,null,CREATE INDEX idx\_pending\_tickets\_privy ON public.pending\_tickets USING btree (lower(user\_id))  
public,pending\_tickets,idx\_pending\_tickets\_reservation,false,false,true,true,false,null,null,CREATE INDEX idx\_pending\_tickets\_reservation ON public.pending\_tickets USING btree (reservation\_id)  
public,pending\_tickets,idx\_pending\_tickets\_status,false,false,true,true,false,null,null,CREATE INDEX idx\_pending\_tickets\_status ON public.pending\_tickets USING btree (status)  
public,pending\_tickets,idx\_pending\_tickets\_status\_expires,false,false,true,true,false,(status \= 'pending'::text),null,"CREATE INDEX idx\_pending\_tickets\_status\_expires ON public.pending\_tickets USING btree (status, expires\_at) WHERE (status \= 'pending'::text)"  
public,pending\_tickets,idx\_pending\_tickets\_wallet\_lower,false,false,true,true,false,null,null,CREATE INDEX idx\_pending\_tickets\_wallet\_lower ON public.pending\_tickets USING btree (lower(wallet\_address))  
public,pending\_tickets,pending\_tickets\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX pending\_tickets\_pkey ON public.pending\_tickets USING btree (id)  
public,pending\_tickets,ux\_pending\_tickets\_reservation\_id,false,true,true,true,false,null,null,CREATE UNIQUE INDEX ux\_pending\_tickets\_reservation\_id ON public.pending\_tickets USING btree (reservation\_id)  
public,platform\_statistics,platform\_statistics\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX platform\_statistics\_pkey ON public.platform\_statistics USING btree (id)  
public,profiles,idx\_profiles\_canonical\_user\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_profiles\_canonical\_user\_id ON public.profiles USING btree (canonical\_user\_id)  
public,profiles,profiles\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX profiles\_pkey ON public.profiles USING btree (id)  
public,purchase\_idempotency,purchase\_idempotency\_idempotency\_key\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX purchase\_idempotency\_idempotency\_key\_key ON public.purchase\_idempotency USING btree (idempotency\_key)  
public,purchase\_idempotency,purchase\_idempotency\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX purchase\_idempotency\_pkey ON public.purchase\_idempotency USING btree (id)  
public,purchase\_requests,purchase\_requests\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX purchase\_requests\_pkey ON public.purchase\_requests USING btree (request\_id)  
public,reservations,reservations\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX reservations\_pkey ON public.reservations USING btree (id)  
public,rng\_logs,idx\_rng\_logs\_competition\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_rng\_logs\_competition\_id ON public.rng\_logs USING btree (competition\_id)  
public,rng\_logs,idx\_rng\_logs\_competition\_type,false,false,true,true,false,null,null,CREATE INDEX idx\_rng\_logs\_competition\_type ON public.rng\_logs USING btree (competition\_type)  
public,rng\_logs,idx\_rng\_logs\_is\_winner,false,false,true,true,false,(is\_winner \= true),null,CREATE INDEX idx\_rng\_logs\_is\_winner ON public.rng\_logs USING btree (is\_winner) WHERE (is\_winner \= true)  
public,rng\_logs,idx\_rng\_logs\_outcome,false,false,true,true,false,null,null,CREATE INDEX idx\_rng\_logs\_outcome ON public.rng\_logs USING btree (outcome)  
public,rng\_logs,idx\_rng\_logs\_timestamp,false,false,true,true,false,null,null,"CREATE INDEX idx\_rng\_logs\_timestamp ON public.rng\_logs USING btree (""timestamp"" DESC)"  
public,rng\_logs,rng\_logs\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX rng\_logs\_pkey ON public.rng\_logs USING btree (id)  
public,site\_metadata,site\_metadata\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX site\_metadata\_pkey ON public.site\_metadata USING btree (id)  
public,site\_stats,site\_stats\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX site\_stats\_pkey ON public.site\_stats USING btree (id)  
public,sub\_account\_balances,idx\_sub\_account\_balances\_canonical,false,false,true,true,false,null,null,CREATE INDEX idx\_sub\_account\_balances\_canonical ON public.sub\_account\_balances USING btree (canonical\_user\_id)  
public,sub\_account\_balances,idx\_sub\_account\_balances\_currency,false,false,true,true,false,null,null,CREATE INDEX idx\_sub\_account\_balances\_currency ON public.sub\_account\_balances USING btree (currency)  
public,sub\_account\_balances,idx\_sub\_account\_balances\_privy\_user\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_sub\_account\_balances\_privy\_user\_id ON public.sub\_account\_balances USING btree (privy\_user\_id)  
public,sub\_account\_balances,idx\_sub\_account\_balances\_user\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_sub\_account\_balances\_user\_id ON public.sub\_account\_balances USING btree (user\_id)  
public,sub\_account\_balances,idx\_sub\_account\_balances\_wallet\_address,false,false,true,true,false,null,null,CREATE INDEX idx\_sub\_account\_balances\_wallet\_address ON public.sub\_account\_balances USING btree (wallet\_address)  
public,sub\_account\_balances,idx\_sub\_bal\_wallet\_currency,false,false,true,true,false,null,null,"CREATE INDEX idx\_sub\_bal\_wallet\_currency ON public.sub\_account\_balances USING btree (wallet\_address, currency, last\_updated DESC)"  
public,sub\_account\_balances,idx\_sub\_balances\_cuid\_currency,false,false,true,true,false,null,null,"CREATE INDEX idx\_sub\_balances\_cuid\_currency ON public.sub\_account\_balances USING btree (canonical\_user\_id, currency)"  
public,sub\_account\_balances,sub\_account\_balances\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX sub\_account\_balances\_pkey ON public.sub\_account\_balances USING btree (id)  
public,sub\_account\_balances,uniq\_sub\_account\_balances\_cuid\_currency,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX uniq\_sub\_account\_balances\_cuid\_currency ON public.sub\_account\_balances USING btree (canonical\_user\_id, currency)"  
public,sub\_account\_balances,uq\_sub\_balances\_cuid\_currency,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX uq\_sub\_balances\_cuid\_currency ON public.sub\_account\_balances USING btree (canonical\_user\_id, currency)"  
public,sub\_account\_balances,uq\_sub\_balances\_user\_currency,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX uq\_sub\_balances\_user\_currency ON public.sub\_account\_balances USING btree (user\_id, currency)"  
public,sub\_account\_balances,uq\_subacct\_can\_user\_currency,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX uq\_subacct\_can\_user\_currency ON public.sub\_account\_balances USING btree (canonical\_user\_id, currency)"  
public,testimonials,testimonials\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX testimonials\_pkey ON public.testimonials USING btree (id)  
public,tickets,idx\_tickets\_canonical\_user,false,false,true,true,false,null,null,CREATE INDEX idx\_tickets\_canonical\_user ON public.tickets USING btree (canonical\_user\_id)  
public,tickets,idx\_tickets\_canonical\_user\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_tickets\_canonical\_user\_id ON public.tickets USING btree (canonical\_user\_id)  
public,tickets,idx\_tickets\_comp\_status\_active,false,false,true,true,false,null,null,"CREATE INDEX idx\_tickets\_comp\_status\_active ON public.tickets USING btree (competition\_id, status, is\_active)"  
public,tickets,idx\_tickets\_comp\_ticket,false,false,true,true,false,null,null,"CREATE INDEX idx\_tickets\_comp\_ticket ON public.tickets USING btree (competition\_id, ticket\_number)"  
public,tickets,idx\_tickets\_competition,false,false,true,true,false,null,null,CREATE INDEX idx\_tickets\_competition ON public.tickets USING btree (competition\_id)  
public,tickets,idx\_tickets\_competition\_status\_tn,false,false,true,true,false,null,null,"CREATE INDEX idx\_tickets\_competition\_status\_tn ON public.tickets USING btree (competition\_id, status, ticket\_number)"  
public,tickets,idx\_tickets\_cuid\_purchased\_at,false,false,true,true,false,null,null,"CREATE INDEX idx\_tickets\_cuid\_purchased\_at ON public.tickets USING btree (canonical\_user\_id, purchased\_at DESC)"  
public,tickets,idx\_tickets\_purchased\_at,false,false,true,true,false,null,null,CREATE INDEX idx\_tickets\_purchased\_at ON public.tickets USING btree (purchased\_at)  
public,tickets,idx\_tickets\_status,false,false,true,true,false,null,null,CREATE INDEX idx\_tickets\_status ON public.tickets USING btree (status)  
public,tickets,idx\_tickets\_user\_comp,false,false,true,true,false,null,null,"CREATE INDEX idx\_tickets\_user\_comp ON public.tickets USING btree (canonical\_user\_id, competition\_id)"  
public,tickets,idx\_tickets\_user\_id\_lower,false,false,true,true,false,(user\_id IS NOT NULL),null,CREATE INDEX idx\_tickets\_user\_id\_lower ON public.tickets USING btree (lower(user\_id)) WHERE (user\_id IS NOT NULL)  
public,tickets,idx\_tickets\_wallet,false,false,true,true,false,null,null,CREATE INDEX idx\_tickets\_wallet ON public.tickets USING btree (wallet\_address)  
public,tickets,tickets\_competition\_id\_ticket\_number\_key,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX tickets\_competition\_id\_ticket\_number\_key ON public.tickets USING btree (competition\_id, ticket\_number)"  
public,tickets,tickets\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX tickets\_pkey ON public.tickets USING btree (id)  
public,tickets\_sold,tickets\_sold\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX tickets\_sold\_pkey ON public.tickets\_sold USING btree (competition\_id, ticket\_number)"  
public,user\_notifications,idx\_user\_notifications\_user\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_user\_notifications\_user\_id ON public.user\_notifications USING btree (user\_id)  
public,user\_notifications,user\_notifications\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX user\_notifications\_pkey ON public.user\_notifications USING btree (id)  
public,user\_transactions,idx\_user\_transactions\_canonical,false,false,true,true,false,null,null,CREATE INDEX idx\_user\_transactions\_canonical ON public.user\_transactions USING btree (canonical\_user\_id)  
public,user\_transactions,idx\_user\_transactions\_competition\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_user\_transactions\_competition\_id ON public.user\_transactions USING btree (competition\_id)  
public,user\_transactions,idx\_user\_transactions\_created\_at,false,false,true,true,false,null,null,CREATE INDEX idx\_user\_transactions\_created\_at ON public.user\_transactions USING btree (created\_at DESC)  
public,user\_transactions,idx\_user\_transactions\_identifiers,false,false,true,true,false,null,null,"CREATE INDEX idx\_user\_transactions\_identifiers ON public.user\_transactions USING btree (canonical\_user\_id, user\_id, user\_privy\_id)"  
public,user\_transactions,idx\_user\_transactions\_status,false,false,true,true,false,null,null,CREATE INDEX idx\_user\_transactions\_status ON public.user\_transactions USING btree (status)  
public,user\_transactions,idx\_user\_transactions\_tx\_id,false,false,true,true,false,null,null,CREATE INDEX idx\_user\_transactions\_tx\_id ON public.user\_transactions USING btree (tx\_id)  
public,user\_transactions,idx\_user\_transactions\_type,false,false,true,true,false,null,null,CREATE INDEX idx\_user\_transactions\_type ON public.user\_transactions USING btree (type)  
public,user\_transactions,idx\_user\_transactions\_type\_txid,false,false,true,true,false,null,null,"CREATE INDEX idx\_user\_transactions\_type\_txid ON public.user\_transactions USING btree (type, tx\_id)"  
public,user\_transactions,idx\_user\_transactions\_user,false,false,true,true,false,null,null,CREATE INDEX idx\_user\_transactions\_user ON public.user\_transactions USING btree (user\_id)  
public,user\_transactions,idx\_user\_transactions\_user\_comp,false,false,true,true,false,null,null,"CREATE INDEX idx\_user\_transactions\_user\_comp ON public.user\_transactions USING btree (canonical\_user\_id, competition\_id)"  
public,user\_transactions,idx\_user\_transactions\_wallet,false,false,true,true,false,null,null,CREATE INDEX idx\_user\_transactions\_wallet ON public.user\_transactions USING btree (wallet\_address)  
public,user\_transactions,idx\_user\_transactions\_wallet\_credited,false,false,true,true,false,null,null,"CREATE INDEX idx\_user\_transactions\_wallet\_credited ON public.user\_transactions USING btree (canonical\_user\_id, created\_at DESC)"  
public,user\_transactions,idx\_user\_tx\_competition\_completed,false,false,true,true,false,(status \= 'completed'::text),null,"CREATE INDEX idx\_user\_tx\_competition\_completed ON public.user\_transactions USING btree (competition\_id, created\_at DESC) WHERE (status \= 'completed'::text)"  
public,user\_transactions,idx\_user\_tx\_topup\_txid\_unique,false,true,true,true,false,(type \= 'topup'::text),null,CREATE UNIQUE INDEX idx\_user\_tx\_topup\_txid\_unique ON public.user\_transactions USING btree (tx\_id) WHERE (type \= 'topup'::text)  
public,user\_transactions,idx\_user\_tx\_user\_created,false,false,true,true,false,null,null,"CREATE INDEX idx\_user\_tx\_user\_created ON public.user\_transactions USING btree (canonical\_user\_id, created\_at)"  
public,user\_transactions,idx\_ut\_status\_type,false,false,true,true,false,null,null,"CREATE INDEX idx\_ut\_status\_type ON public.user\_transactions USING btree (status, type)"  
public,user\_transactions,user\_transactions\_charge\_id\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX user\_transactions\_charge\_id\_key ON public.user\_transactions USING btree (charge\_id)  
public,user\_transactions,user\_transactions\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX user\_transactions\_pkey ON public.user\_transactions USING btree (id)  
public,user\_transactions,user\_transactions\_webhook\_ref\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX user\_transactions\_webhook\_ref\_key ON public.user\_transactions USING btree (webhook\_ref)  
public,users,users\_email\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX users\_email\_key ON public.users USING btree (email)  
public,users,users\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX users\_pkey ON public.users USING btree (id)  
public,users,users\_privy\_id\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX users\_privy\_id\_key ON public.users USING btree (privy\_id)  
public,users,users\_user\_id\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX users\_user\_id\_key ON public.users USING btree (user\_id)  
public,users,users\_wallet\_address\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX users\_wallet\_address\_key ON public.users USING btree (wallet\_address)  
public,wallet\_balances\_table\_backup,idx\_wallet\_balances\_canonical,false,false,true,true,false,null,null,CREATE INDEX idx\_wallet\_balances\_canonical ON public.wallet\_balances\_table\_backup USING btree (canonical\_user\_id)  
public,wallet\_balances\_table\_backup,wallet\_balances\_canonical\_user\_id\_key,false,true,true,true,false,null,null,CREATE UNIQUE INDEX wallet\_balances\_canonical\_user\_id\_key ON public.wallet\_balances\_table\_backup USING btree (canonical\_user\_id)  
public,wallet\_balances\_table\_backup,wallet\_balances\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX wallet\_balances\_pkey ON public.wallet\_balances\_table\_backup USING btree (id)  
public,webhook\_logs,idx\_webhook\_logs\_status,false,false,true,true,false,null,null,CREATE INDEX idx\_webhook\_logs\_status ON public.webhook\_logs USING btree (status)  
public,webhook\_logs,idx\_webhook\_logs\_type\_created,false,false,true,true,false,null,null,"CREATE INDEX idx\_webhook\_logs\_type\_created ON public.webhook\_logs USING btree (webhook\_type, created\_at DESC)"  
public,webhook\_logs,webhook\_logs\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX webhook\_logs\_pkey ON public.webhook\_logs USING btree (id)  
public,winners,idx\_winners\_competition,false,false,true,true,false,null,null,CREATE INDEX idx\_winners\_competition ON public.winners USING btree (competition\_id)  
public,winners,winners\_competition\_prize\_position\_key,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX winners\_competition\_prize\_position\_key ON public.winners USING btree (competition\_id, prize\_position)"  
public,winners,winners\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX winners\_pkey ON public.winners USING btree (id)  
realtime,messages,messages\_inserted\_at\_topic\_index,false,false,true,true,false,((extension \= 'broadcast'::text) AND (private IS TRUE)),null,"CREATE INDEX messages\_inserted\_at\_topic\_index ON ONLY realtime.messages USING btree (inserted\_at DESC, topic) WHERE ((extension \= 'broadcast'::text) AND (private IS TRUE))"  
realtime,messages,messages\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX messages\_pkey ON ONLY realtime.messages USING btree (id, inserted\_at)"  
realtime,messages\_2026\_01\_30,messages\_2026\_01\_30\_inserted\_at\_topic\_idx,false,false,true,true,false,((extension \= 'broadcast'::text) AND (private IS TRUE)),null,"CREATE INDEX messages\_2026\_01\_30\_inserted\_at\_topic\_idx ON realtime.messages\_2026\_01\_30 USING btree (inserted\_at DESC, topic) WHERE ((extension \= 'broadcast'::text) AND (private IS TRUE))"  
realtime,messages\_2026\_01\_30,messages\_2026\_01\_30\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX messages\_2026\_01\_30\_pkey ON realtime.messages\_2026\_01\_30 USING btree (id, inserted\_at)"  
realtime,messages\_2026\_01\_31,messages\_2026\_01\_31\_inserted\_at\_topic\_idx,false,false,true,true,false,((extension \= 'broadcast'::text) AND (private IS TRUE)),null,"CREATE INDEX messages\_2026\_01\_31\_inserted\_at\_topic\_idx ON realtime.messages\_2026\_01\_31 USING btree (inserted\_at DESC, topic) WHERE ((extension \= 'broadcast'::text) AND (private IS TRUE))"  
realtime,messages\_2026\_01\_31,messages\_2026\_01\_31\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX messages\_2026\_01\_31\_pkey ON realtime.messages\_2026\_01\_31 USING btree (id, inserted\_at)"  
realtime,messages\_2026\_02\_01,messages\_2026\_02\_01\_inserted\_at\_topic\_idx,false,false,true,true,false,((extension \= 'broadcast'::text) AND (private IS TRUE)),null,"CREATE INDEX messages\_2026\_02\_01\_inserted\_at\_topic\_idx ON realtime.messages\_2026\_02\_01 USING btree (inserted\_at DESC, topic) WHERE ((extension \= 'broadcast'::text) AND (private IS TRUE))"  
realtime,messages\_2026\_02\_01,messages\_2026\_02\_01\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX messages\_2026\_02\_01\_pkey ON realtime.messages\_2026\_02\_01 USING btree (id, inserted\_at)"  
realtime,messages\_2026\_02\_02,messages\_2026\_02\_02\_inserted\_at\_topic\_idx,false,false,true,true,false,((extension \= 'broadcast'::text) AND (private IS TRUE)),null,"CREATE INDEX messages\_2026\_02\_02\_inserted\_at\_topic\_idx ON realtime.messages\_2026\_02\_02 USING btree (inserted\_at DESC, topic) WHERE ((extension \= 'broadcast'::text) AND (private IS TRUE))"  
realtime,messages\_2026\_02\_02,messages\_2026\_02\_02\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX messages\_2026\_02\_02\_pkey ON realtime.messages\_2026\_02\_02 USING btree (id, inserted\_at)"  
realtime,messages\_2026\_02\_03,messages\_2026\_02\_03\_inserted\_at\_topic\_idx,false,false,true,true,false,((extension \= 'broadcast'::text) AND (private IS TRUE)),null,"CREATE INDEX messages\_2026\_02\_03\_inserted\_at\_topic\_idx ON realtime.messages\_2026\_02\_03 USING btree (inserted\_at DESC, topic) WHERE ((extension \= 'broadcast'::text) AND (private IS TRUE))"  
realtime,messages\_2026\_02\_03,messages\_2026\_02\_03\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX messages\_2026\_02\_03\_pkey ON realtime.messages\_2026\_02\_03 USING btree (id, inserted\_at)"  
realtime,messages\_2026\_02\_04,messages\_2026\_02\_04\_inserted\_at\_topic\_idx,false,false,true,true,false,((extension \= 'broadcast'::text) AND (private IS TRUE)),null,"CREATE INDEX messages\_2026\_02\_04\_inserted\_at\_topic\_idx ON realtime.messages\_2026\_02\_04 USING btree (inserted\_at DESC, topic) WHERE ((extension \= 'broadcast'::text) AND (private IS TRUE))"  
realtime,messages\_2026\_02\_04,messages\_2026\_02\_04\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX messages\_2026\_02\_04\_pkey ON realtime.messages\_2026\_02\_04 USING btree (id, inserted\_at)"  
realtime,messages\_2026\_02\_05,messages\_2026\_02\_05\_inserted\_at\_topic\_idx,false,false,true,true,false,((extension \= 'broadcast'::text) AND (private IS TRUE)),null,"CREATE INDEX messages\_2026\_02\_05\_inserted\_at\_topic\_idx ON realtime.messages\_2026\_02\_05 USING btree (inserted\_at DESC, topic) WHERE ((extension \= 'broadcast'::text) AND (private IS TRUE))"  
realtime,messages\_2026\_02\_05,messages\_2026\_02\_05\_pkey,true,true,true,true,false,null,null,"CREATE UNIQUE INDEX messages\_2026\_02\_05\_pkey ON realtime.messages\_2026\_02\_05 USING btree (id, inserted\_at)"  
realtime,schema\_migrations,schema\_migrations\_pkey,true,true,true,true,false,null,null,CREATE UNIQUE INDEX schema\_migrations\_pkey ON realtime.schema\_migrations USING btree (version)  
realtime,subscription,ix\_realtime\_subscription\_entity,false,false,true,true,false,null,null,CREATE INDEX ix\_realtime\_subscription\_entity ON realtime.subscription USING btree (entity)  
realtime,subscription,pk\_subscription,true,true,true,true,false,null,null,CREATE UNIQUE INDEX pk\_subscription ON realtime.subscription USING btree (id)  
realtime,subscription,subscription\_subscription\_id\_entity\_filters\_key,false,true,true,true,false,null,null,"CREATE UNIQUE INDEX subscription\_subscription\_id\_entity\_filters\_key ON realtime.subscription USING btree (subscription\_id, entity, filters)"
