...
async function dropAllOverloads(fname) {
  ...
  for (const f of funcs.rows) {
    // Strip param names and defaults, keep only types
...
async function run() {
  ...
  `);
  console.log('Functions with usdc_balance before cleanup:', remaining.rows.length);
  remaining.rows.forEach(r => console.log('  -', r.proname));

  // Drop ALL overloads of these functions (including any we created)
  ...
  for (const fname of toDropCompletely) {
    await dropAllOverloads(fname);
  }

  console.log('\nRecreating functions fresh...\n');

  // credit_user_balance
  await client.query(`
    CREATE FUNCTION public.credit_user_balance(p_user_id uuid, p_amount numeric)
  ...
  // get_custody_wallet_summary
  await client.query(`
    CREATE FUNCTION public.get_custody_wallet_summary()
  ...
  // get_user_balance
  await client.query(`
    CREATE FUNCTION public.get_user_balance(p_user_identifier text DEFAULT NULL, p_canonical_user_id text DEFAULT NULL)
  ...
  // credit_sub_account_with_bonus
  await client.query(`
    CREATE FUNCTION public.credit_sub_account_with_bonus(p_canonical_user_id text, p_amount numeric, p_currency text DEFAULT 'USD')
    RETURNS numeric LANGUAGE plpgsql SECURITY DEFINER AS $func$
  ...
  // get_user_dashboard_entries
  await client.query(`
    CREATE FUNCTION public.get_user_dashboard_entries(p_canonical_user_id text, p_include_pending boolean DEFAULT false)
