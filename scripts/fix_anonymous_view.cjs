  ...
  try {
    ...
    sample.rows.forEach((r) => {
      const wallet = r.wallet_address
        ? r.wallet_address.substring(0, 10) + "..."
        : "null";
      const cuid = r.canonical_user_id
        ? r.canonical_user_id.substring(0, 15) + "..."
        : "null";
      console.log(
        `  wallet: ${wallet} | cuid: ${cuid} | username: ${r.username || "NULL"}`,
      );
    });
      ...
      for (const r of missing.rows) {
        ...
        if (fullEntry.rows[0]) {
          ...
          if (
            ...
            console.log(
              "    ❌ ORPHAN ENTRY (no user identifiers) - deleting...",
            );
            await client.query(`DELETE FROM joincompetition WHERE id = $1`, [
              r.id,
            ]);
