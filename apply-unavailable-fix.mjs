...
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_ROLE_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im10aHdmbGRjanZweGp0bXJxa3FtIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTczNTkxOTU3NCwiZXhwIjoyMDUxNDk1NTc0fQ.JfncYvQICTb7RjBP24LBiGFKHKMZPfFMZ4aXHX8xbYQ"
);
...
try {
  ...
  if (error) {
    ...
    if (tickets?.length === unavailable?.length) {
      ...
      console.log(`   Expected: ${tickets?.length}, Got: ${unavailable?.length}`);
    }
  }
} catch (e) {
